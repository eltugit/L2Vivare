...
import gabriel.dressmeEngine.xml.dataHolder.DressMeAgathionHolder;
...
public final class DressMeAgathionParser extends AbstractFileParser<DressMeAgathionHolder> {
    private final String SHIELD_FILE_PATH = Config.DATAPACK_ROOT + "/data/xml/sunrise/dressme/agathion.xml";

    private static final DressMeAgathionParser _instance = new DressMeAgathionParser();

    public static DressMeAgathionParser getInstance() {
        return _instance;
    }

    private DressMeAgathionParser() {
        super(DressMeAgathionHolder.getInstance());
    }
            ...
            for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling()) {
                if (n.getNodeName().equalsIgnoreCase("list")) {
                    for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                        if (d.getNodeName().equalsIgnoreCase("agathion")) {
                            ...
                            getHolder().addAgathion(new DressMeAgathionData(number, id, name, itemId, itemCount));
                        }
                    }
                }
            }
