...
public class AccessGranterEX {
    ...
    public AccessGranterEX(L2PcInstance player, L2Party party, L2Clan clan, L2CommandChannel commandChannel) {
        this.player = player;
        this.party = party;
        this.clan = clan;
    ...
    public AccessGranterEX getRightAccess(L2PcInstance player) {
        if (commandChannel != null) {
            if (player.getParty() != null && player.getParty().getCommandChannel() != null && commandChannel == player.getParty().getCommandChannel())
                return this;
        }
    ...
    public void teleportPlayers(Location npcLocation) {
        Location loc = ExtremeZoneManager.getInstance().getNextZone().getPlayerLocs().get(Rnd.get(ExtremeZoneManager.getInstance().getNextZone().getPlayerLocs().size()));

        if (party != null) {
            for (L2PcInstance member : party.getMembers()) {
                if (canEnter(member, npcLocation)) {
                    ExtremeZoneManager.getInstance().teleportPlayerIntoInstance(member, loc);
                    deleteLines(member);
                }
            }
        }
    ...
    private void deleteLines(L2PcInstance player) {
        if (GabConfig.EXTREME_EVENT_DRAW_LINES) {
            if (GabConfig.EXTREME_EVENT_RADIUS_CHECK) {
                ExtremeZoneManager.getInstance().drawn(player, true, true);
            } else {
                ExtremeZoneManager.getInstance().drawn(player, true, false);
            }
        }
    }

    private boolean canEnter(L2PcInstance player, Location npcLocation) {
        return (!GabConfig.EXTREME_EVENT_RADIUS_CHECK && player.isInsideZone(ZoneIdType.EXTREME_CHECKER)) ||
                (GabConfig.EXTREME_EVENT_RADIUS_CHECK && player.isInsideRadius(npcLocation, GabConfig.EXTREME_EVENT_RADIUS_VALUE, true, false));
    }
}
