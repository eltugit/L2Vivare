...
public class LOLTournamentManager {
    private static final Logger _log = LoggerFactory.getLogger(LOLTournamentManager.class);

    private static LOLTournamentManager instance;
    ...
    private LOLTournamentTask _task;
    private Future<?> matchMaking;
    private int instancesIdGen = 160000;
    ...
    public void startMatchMaking() {
        ...
        }, 20000, Config.ARENA_CALL_INTERVAL_LOL);
    }

    private int MAX_ELO_BETWEEN_PLAYERS = 500;

    public void matchMaking() {
        synchronized (teams) {
            Map<Integer, List<TTeam>> active = new HashMap<>();

            for (Map.Entry<Integer, List<TTeam>> team : LOLTournamentManager.getInstance().getTeams().entrySet()) {
                int qnt = team.getKey();
                List<TTeam> tteamTemp = team.getValue().stream().filter(e -> !e.isInGame()).collect(Collectors.toList());
//                if(tteamTemp.size() < 2)
//                    continue;
                tteamTemp.sort(Comparator.comparingInt(e->e.getTeamElo(qnt)));
                Collections.reverse(tteamTemp);

                List<TTeam> tteam = new LinkedList<>();
                TTeam temporaryTeam = null;

                loop: for (TTeam tTeam : tteamTemp) {
                    if(temporaryTeam == null)
                        temporaryTeam = tTeam;
                    else{
                        if(Math.abs(tTeam.getTeamElo(qnt) - temporaryTeam.getTeamElo(qnt)) <= MAX_ELO_BETWEEN_PLAYERS){
                            tteam.add(temporaryTeam);
                            tteam.add(tTeam);
                            break loop;
                        }
                        temporaryTeam = tTeam;
            ...
            for (Map.Entry<Integer, List<TTeam>> entry : active.entrySet()) {
                ...
                for (TTeam tTeam : value) {
                    LOLTournamentManager.getInstance().getTeams().get(qnt).remove(tTeam);
    ...
    private boolean inFight(TTeam toCheck) {
        boolean found = false;
        for (List<TFight> value : LOLTournamentManager.getInstance().getFights().values()) {
    ...
    private void createNewFight(List<TTeam> team, int qnt) {
        ...
        TFight fight = new TFight(qnt, instanceId, true);

        LOLTournamentManager.getInstance().checkFightExist(qnt);
        ...
        LOLTournamentManager.getInstance().getFights().get(qnt).add(fight);
    ...
    public void removeTeam(int qnt, TTeam team) {
        List<TTeam> toRemoveList = LOLTournamentManager.getInstance().getTeams().get(qnt);
        TTeam temp = null;
        ...
        if (temp != null)
            LOLTournamentManager.getInstance().getTeams().get(qnt).remove(temp);

        try {
            LOLTournamentManager.getInstance().getTeams().get(qnt).remove(team);
        } catch (Exception e) {
            //
        }
...
//        instance = null;
    ...
    public static LOLTournamentManager getInstance() {
        if (instance == null)
            instance = new LOLTournamentManager();
        return instance;
    }
    ...
    private LOLTournamentManager() {
        startMatchMaking();
        _log.info("Custom LOL Tournament by Gabriel has been started");
    }
    ...
    public LOLTournamentTask get_task() {
        return _task;
    }

    public void set_task(LOLTournamentTask _task) {
        this._task = _task;
    }
    ...
    public boolean registerTeam(L2PcInstance player, int qntPlayers) {
        ...
        if (party != null) {
            ...
            for (L2PcInstance partyMember : party.getMembers()) {
                if (Config.ARENA_EVENT_PREVENT_IP_LOL && checkIfAlreadyRegistered(partyMember, qntPlayers)) {
            ...
            if (Config.ARENA_EVENT_PREVENT_IP_LOL && checkIfAlreadyRegistered(player, qntPlayers)) {
    ...
    public boolean checkIfAlreadyRegisteredInTour(L2PcInstance player, int qntPlayers){
        checkFightExist(qntPlayers);
        checkTeamExist(qntPlayers);
        L2Party party = player.getParty();
        boolean found = false;
        if (party != null) {
            for (L2PcInstance partyMember : party.getMembers()) {
                if (Config.ARENA_EVENT_PREVENT_IP_LOL && checkIfAlreadyRegistered(player, qntPlayers)) {
                    player.sendPacket(new CreatureSay(player.getObjectId(), Say2.BATTLEFIELD, player.getName(), String.format("Member %s is already registered", partyMember.getName())));
                    found = true;
                }
            }
        }else{
    ...
    public boolean checkIfAlreadyRegistered(L2PcInstance player, int qnt) {
        for (List<TTeam> value : teams.values()) {
            for (TTeam team : value) {
                for (L2PcInstance teamPlayer : team.getPlayers()) {
                    if (Config.ARENA_EVENT_PREVENT_IP_LOL && Tools.isDualBox(teamPlayer, player)) {
                        if(qnt == 1){
    ...
    public boolean checkIfRegistered(L2PcInstance player, int qnt) {
        checkFightExist(qnt);
        checkTeamExist(qnt);
