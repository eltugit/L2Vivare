 ...
 */
public class TournamentRestrictions implements GabrielRestrictions {
    private static final class SingletonHolder {
        private static final TournamentRestrictions INSTANCE = new TournamentRestrictions();
    }



    public static TournamentRestrictions getInstance() {
    ...
    public boolean checkInside(L2Character activeChar, L2Character target) {
        return activeChar.isInTournament() && target.isInTournament();
    }
    ...
    public boolean isFriend(L2Character activeChar, L2Character target) {
        if (checkInside(activeChar, target)) {
            if (BasicGabsRestrictions.checkSameParty(activeChar, target))
                return true;
            else if (activeChar.gettTeam() == target.gettTeam())
                return true;
    ...
    public boolean doAttack(L2Character activeChar, L2Character target) {
        ...
        if (isL2PcInstance(activeChar, target) && checkInside(activeChar, target)) {
            ...
            if (activeChar.gettTeam() != target.gettTeam()) {
                return false;
            }
        }
        return false;
