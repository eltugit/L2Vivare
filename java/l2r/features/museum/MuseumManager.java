...
public class MuseumManager
{
	...
	private HashMap<Integer, String> _categoryNames;
	private HashMap<Integer, MuseumCategory> _categories;
	private HashMap<Integer, ArrayList<MuseumCategory>> _categoriesByCategoryId;
	private HashMap<Integer, ArrayList<RewardClass>> _playersWithReward;
...
//        _playersWithReward = new HashMap<>();
//        loadCategories();
        ThreadPoolManager.getInstance().scheduleGeneral(new UpdateStats(RefreshTime.Total),10000L);
//        ThreadPoolManager.getInstance().scheduleGeneral(new UpdateStats(RefreshTime.Monthly),500L);
//        ThreadPoolManager.getInstance().scheduleGeneral(new UpdateStats(RefreshTime.Weekly),500L);
//        ThreadPoolManager.getInstance().scheduleGeneral(new UpdateStats(RefreshTime.Daily),500L);
    }
	
	public void giveRewards()
	{
		final ArrayList<Integer> withReward = new ArrayList<>();
		for (final Map.Entry<Integer, ArrayList<RewardClass>> entry : _playersWithReward.entrySet())
		{
			...
			if (player != null)
			{
				...
				final ArrayList<RewardClass> cats = entry.getValue();
				for (final RewardClass rewardClass : cats)
				{
				    int catId = rewardClass.getCategoryId();
				    int rank = rewardClass.getRank();
					...
					else
					{
						...
						{
                            List<MuseumReward> list = cat.getRewards().get(rank);
                            if(list == null){
                                System.out.println("Rank reward for rank: "+rank +" is null! doesnt exist ("+cat.getCategoryName()+")");
                                continue;
                            }

                            for (final MuseumReward reward : list)
                            {
	...
	public void giveReward(final L2PcInstance player)
	{
		...
		final ArrayList<RewardClass> cats = _playersWithReward.get(player.getObjectId());
		if (cats.size() < 1)
		{
		...
		for (final RewardClass rewardClass : cats)
		{
			...
			final MuseumCategory cat = _categories.get(catId);
			for (final MuseumReward reward : cat.getRewards().get(rank))
			{
				reward.giveReward(player);
			}
		}
	...
	private static class RewardClass{
	    private int rank;
	    private int categoryId;

        public RewardClass(int rank, int categoryId) {
            this.rank = rank;
					...
					try (final ResultSet rset = statement.executeQuery())
					{
						while (rset.next())
						{
							...
							if (hasReward)
							{
								...
								{
									_playersWithReward.put(objectId, new ArrayList<RewardClass>());
								}
								_playersWithReward.get(objectId).add(new RewardClass(rankIndex, (cat.getCategoryId() * 256) + cat.getTypeId()));
							}
				...
				if (d.getNodeName().equalsIgnoreCase("category"))
				{
					...
					for (Node h = d.getFirstChild(); h != null; h = h.getNextSibling())
					{
						if (h.getNodeName().equalsIgnoreCase("type"))
						{
							...
							final Map<Integer, List<MuseumReward>> rewards = new LinkedHashMap<>();
							int rewardId = 0;
							for (Node a = h.getFirstChild(); a != null; a = a.getNextSibling())
							{
								...
								if (a.getNodeName().equalsIgnoreCase("reward"))
								{
									...
									int maxCount = 0;
									double chance = 0.0;
									if (a.getAttributes().getNamedItem("rank") != null)
									{
                                        rank = Integer.parseInt(a.getAttributes().getNamedItem("rank").getNodeValue());
									}
									...
									rewards.computeIfAbsent(rank, k -> new LinkedList<>()).add(new MuseumReward(rewardId, rewardType, itemId, minCount, maxCount, chance));
									++rewardId;
								}
							}
