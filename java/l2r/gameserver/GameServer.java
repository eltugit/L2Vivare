...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		// load script engines
		// TODO GABRIEL MOVE ALL JAVA SCRIPTS TO SOURCE!
		...
		// TODO GABRIEL Check What this does
		...
		/*
		 * try { //TODO GABRIEL MOVE ALL JAVA SCRIPTS TO SOURCE! printSection("Datapack Scripts"); if (!Config.ALT_DEV_NO_HANDLERS || !Config.ALT_DEV_NO_QUESTS) { L2ScriptEngineManager.getInstance().executeScriptList(new File(Config.DATAPACK_ROOT, "data/scripts.ini")); } } catch (IOException ioe) {
		 * _log.error(getClass().getSimpleName() + ": Failed loading scripts.ini, scripts are not going to be loaded!"); }
		 */
		GabrielScriptsLoader.getInstance();
		...
		printSection("rGuard Protection");
		Protection.Init();
		...
		printSection("Gabriel System");
		GabrielModsInit.getInstance();
		DressMeLoader.load();
