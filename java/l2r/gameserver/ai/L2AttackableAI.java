...
import static l2r.gameserver.enums.CtrlIntention.*;
...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target.isDoor())
		{
			if(getActiveChar().isPlayable() && CSKOTHEvent.canAttackDoor(getActiveChar()))
				return true;
			return false;
		}
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		if ((intention == AI_INTENTION_IDLE) || (intention == AI_INTENTION_ACTIVE))
		{
			...
			// vGodFather if intention is active we should avoid extra checks
			if (!npc.isAlikeDead() && (intention != AI_INTENTION_ACTIVE))
			{
	...
	protected void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			// vGodFather if mob is not aggressive we should avoid these checks
	...
	protected boolean cast(L2Skill sk)
	{
		if (sk == null)
		{
			return false;
	 ...
	 */
	protected static boolean checkSkillCastConditions(L2Attackable caster, L2Skill skill)
	{
		return checkSkillCastConditions(caster, null, skill);
	}
	
	protected static boolean checkSkillCastConditions(L2Attackable caster, L2Object target, L2Skill skill)
	{
		...
		// vGodFather: finally check for skill range and geodata
		if ((target != null) && (!Util.checkIfInRange(skill.getCastRange(), caster, target, true) || !GeoData.getInstance().canSeeTarget(caster, target)))
		{
			return false;
		}
		return true;
	...
	protected void targetReconsider()
	{
		double dist = 0;
		double dist2 = 0;
		int range = 0;
	...
	private void factionCall(L2Attackable me, L2Character target)
	{
		...
		{
			final int factionRange = me.getTemplate().getClanHelpRange() + me.getTemplate().getCollisionRadius();
			...
			for (L2Object obj : me.getKnownList().getKnownCharactersInRadius(factionRange))
			{
				...
				// We will stop if mob does not have ai or is dead
				if (!called.hasAI() || called.isDead() || !getActiveChar().getTemplate().isClan(called.getTemplate().getClans()))
				{
					continue;
				}
	...
	// vGodFather this will fix mobs return to home function
	private void tryToGoHome(L2Attackable npc)
	{
		...
		catch (Exception e)
		{
			...
			// nothing to log
			_log.warn("Problem detected with tryToGoHome function: " + e.getMessage(), e);
			npc.returnHome(true);
		}
	}
