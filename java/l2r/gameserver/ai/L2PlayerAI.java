    ...
    private void thinkCast()
    {
        ...
        if ((_skill.getTargetType() == L2TargetType.GROUND) && (_actor instanceof L2PcInstance))
        {
            if (!GeoData.getInstance().canMove(_actor, ((L2PcInstance) _actor).getCurrentSkillWorldPosition())) {
                if(Math.abs(_actor.getZ() - ((L2PcInstance) _actor).getCurrentSkillWorldPosition().getZ()) > L2PcInstance.SAFE_CAST_HEIGHT_GROUND_SKILL || _actor.getDistance(((L2PcInstance) _actor).getCurrentSkillWorldPosition()) > _skill.getCastRange()) {
