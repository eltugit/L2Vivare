...
public class ServicesBBSManager extends BaseBBSManager
{
	...
	public void cbByPass(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith(_servicesBBSCommand + "_sendMultisell"))
		{
			...
			else
            {
                try
                {
                    ...
                    {
                        activeChar.setQuickVar(QuickVarType.COMMUNITY_SELL.getCommand(), 1);
		...
		else if (command.startsWith(_servicesBBSCommand + "_teleport"))
		{
			...
			if (activeChar.isJailed()  || activeChar.isInCombat()  || activeChar.isInTournament()  || activeChar.isAlikeDead() || activeChar.isInOlympiadMode() || activeChar.inObserverMode() || SunriseEvents.isInEvent(activeChar) || OlympiadManager.getInstance().isRegistered(activeChar))
			{
			...
			try
			{
				...
				if (Conditions.checkPlayerItemCount(activeChar, itemIdToGet, price))
				{
					...
					{
                        PvPZoneManager.handleLeavePvPZone(activeChar, false);
					...
					else
					{
						...
						activeChar.setSkillCast(ThreadPoolManager.getInstance().scheduleGeneral(new Teleport(activeChar, c[0], c[1], c[2]), 1000));
...
//			content = HtmCache.getInstance().getHtm(activeChar, activeChar.getHtmlPrefix(), "data/html/scripts/services/gabriel/symbolmaker.htm");
//			separateAndSend(content, activeChar);
			
			final List<L2Henna> tato = HennaData.getInstance().getHennaList(activeChar.getClassId());
			activeChar.sendPacket(new HennaEquipList(activeChar, tato));
            GabrielCBB.getInstance().parseCommand("gab_services;symbolMaker", activeChar);
		}
...
//			separateAndSend(content, activeChar);
			...
			for (int i = 1; i <= 3; i++)
			{
				final L2Henna henna = activeChar.getHennaEx().getHenna(i);
				if (henna != null)
				{
					hasHennas = true;
				}
...
//            content = HtmCache.getInstance().getHtm(activeChar, activeChar.getHtmlPrefix(), "data/html/scripts/services/gabriel/blacksmith.htm");
//			separateAndSend(content, activeChar);

            activeChar.sendPacket(new ExShowVariationMakeWindow());
            GabrielCBB.getInstance().parseCommand("gab_services;blacksmith", activeChar);
        }
...
//            content = HtmCache.getInstance().getHtm(activeChar, activeChar.getHtmlPrefix(), "data/html/scripts/services/gabriel/warehouse.htm");
//			separateAndSend(content, activeChar);
            activeChar.setQuickVar(QuickVarType.PORTAL_WH.getCommand(), true);

            GenerateHtmls.showPWithdrawWindow(activeChar, null, (byte) 0);
            GabrielCBB.getInstance().parseCommand("gab_services;warehouse", activeChar);
        }
...
//            activeChar.setIsUsingAioWh(true);
		...
		{
			if(activeChar.canBuff > System.currentTimeMillis()){
				activeChar.sendMessage("Please wait a while to buff after reviving.");
				return;
			}
		...
		else if (command.startsWith(_servicesBBSCommand + "_functions_buffer"))
		{
			...
			if ((activeChar.isInCombat() || activeChar.isJailed() || activeChar.isAlikeDead() || activeChar.isInOlympiadMode() || activeChar.inObserverMode() || OlympiadManager.getInstance().isRegistered(activeChar)))
			{
				activeChar.sendMessage("Cannot use at the moment.");
				content = HtmCache.getInstance().getHtm(activeChar, activeChar.getHtmlPrefix(), "data/html/CommunityBoard/services/buffer/main.htm");
				separateAndSend(content, activeChar);
			...
			else if (subCommand[4].startsWith("healme"))
			{
			    if(SunriseEvents.isInEvent(activeChar)) {
			        activeChar.sendMessage("Cannot heal in event!");
                    return;
                }
			...
			else if (subCommand[4].startsWith("autobuff"))
			{
				...
				{
					activeChar.sendMessage("Cannot use this feature at this moment.");
					BufferPacketSender.sendPacket(activeChar, "functions.htm", BufferPacketCategories.COMMUNITY, 1);
					return;
				}
	...
	private class Teleport implements Runnable
	{
		...
		public void run()
		{
            PvPZoneManager.handleLeavePvPZone(_activeChar, false);
