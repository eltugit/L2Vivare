	...
	public void load(L2PcInstance player)
	{
			...
			PreparedStatement ps = con.prepareStatement(SELECT))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					player.getBookmarkEx().getTpbookmarks().put(rs.getInt("Id"), new TeleportBookmark(rs.getInt("Id"), rs.getInt("x"), rs.getInt("y"), rs.getInt("z"), rs.getInt("icon"), rs.getString("tag"), rs.getString("name")));
				}
			}
		}
		catch (Exception e)
