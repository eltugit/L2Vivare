...
public final class MultisellData implements IXmlReader
{
	...
	private final Entry parseEntry(Node n, int entryId, ListContainer list)
	{
		...
		if ((entry.getIngredients().size() == 1) && (entry.getProducts().size() == 1) && (entry.getIngredients().get(0).getId() == 57))
		{
			L2Item item = ItemData.getInstance().getTemplate(entry.getProducts().get(0).getId());
			if (item == null)
			{
				LOGGER.warn(getClass().getSimpleName() + ": [" + list.getListId() + "] Production [" + entry.getProducts().get(0).getId() + "] not found!");
				return null;
			}
			
			if (Config.DEBUG_MULTISELL_PRICES)
			{
				if (item.getReferencePrice() > (entry.getIngredients().get(0).getItemCount() / entry.getProducts().get(0).getItemCount()))
				{
					LOGGER.warn(getClass().getSimpleName() + ": [" + list.getListId() + "] Production '" + item.getName() + "' [" + entry.getProducts().get(0).getId() + "] price is lower than referenced | " + item.getReferencePrice() + " / " + entry.getIngredients().get(0).getItemCount() + " should be " + (item.getReferencePrice() * entry.getProducts().get(0).getItemCount()));
				}
			}
		}
	...
	public final void separateAndSend(int listId, L2PcInstance player, L2Npc npc, boolean inventoryOnly, double productMultiplier, double ingredientMultiplier)
	{
		...
		Broadcast.sendMultisellPath(player, String.valueOf(listId));
	}
