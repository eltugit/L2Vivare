	...
	public Location getTeleToLocation(L2Character activeChar, TeleportWhereType teleportWhere)
	{
		...
		if (activeChar instanceof L2PcInstance)
		{
			...
			if (player.getInstanceId() > 0)
			{
                ...
                Location temp;
                if (inst != null) {
                    loc = inst.getSpawnLocPvP();
                    if (loc != null) {
                        //this will happen if player is inside instance and clicks to village
                        PvPZoneManager.handleLeavePvPZone(player, false);
                        //System.out.println("clicked to village "+PvPZoneManager.playerinInstance);
                        player.setInstanceId(0);

                        return loc;
                    } else {
                        temp = inst.getExitLoc();
                        if (temp != null)
                        {
                            return temp;
                        }
