...
public class RaidBossSpawnManager
{
	...
	{
		final StatsSet info = new StatsSet();
        info.set("boss_id", raidboss.getId());
		info.set("currentHP", raidboss.getCurrentHp());
		info.set("currentMP", raidboss.getCurrentMp());
		info.set("respawnTime", 0L);
	...
	public Map<Integer, StatsSet> getAllBossesStatSets() {
        Map<Integer, StatsSet> sets = new HashMap<>();
        for (Map.Entry<Integer, L2RaidBossInstance> bbb : getBosses().entrySet()) {
            sets.put(bbb.getKey(), getStatsSet(bbb.getValue().getId()));
        }
	    return sets;
    }
