...
public final class TerritoryWarManager implements Siegable
{
    ...
    private final Map<Integer, L2SiegeFlagInstance> _outposts = new ConcurrentHashMap<>();
    ...
    protected void startTerritoryWar()
    {
        ...
        SiegeRankManager.getInstance().resetStart(false);
    ...
    protected void endTerritoryWar()
    {
        ...
        Broadcast.toAllOnlinePlayers(sm);
        SiegeRankManager.getInstance().sendToEveryOne(false);
    ...
    public int getTotalWards(int newOwnerId)
    {
        if (_territoryList.containsKey(newOwnerId))
        {
            Territory terNew = _territoryList.get(newOwnerId);
            return terNew.getOwnedWardIds().isEmpty() ? 0 : terNew.getOwnedWardIds().size();
        }
        return 0;
    }
