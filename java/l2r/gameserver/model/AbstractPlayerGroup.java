...
public abstract class AbstractPlayerGroup
{
    ...
    private Location pvpZoneLocation = null;

    public Location getPvpZoneLocation() {
        return pvpZoneLocation;
    }

    public void setPvpZoneLocation(Location pvpZoneLocation) {
        this.pvpZoneLocation = pvpZoneLocation;
        if(pvpLocReset == null){
            pvpLocReset = ThreadPoolManager.getInstance().scheduleGeneral(() -> {
                setPvpZoneLocation(null);
                pvpLocReset();
            },5*60*1000);
        }
    }

    private Future<?> pvpLocReset = null;

    public void pvpLocReset() {
        if(pvpLocReset != null)
            pvpLocReset.cancel(true);
        this.pvpLocReset = null;
    }
}
