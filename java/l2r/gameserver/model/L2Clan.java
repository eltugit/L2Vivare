...
public class L2Clan implements IIdentifiable, INamable
{
	...
	// SQL queries
	private static final String INSERT_CLAN_DATA = "INSERT INTO clan_data (clan_id,clan_name,clan_level,hasCastle,blood_alliance_count,blood_oath_count,ally_id,ally_name,leader_id,crest_id,crest_large_id,ally_crest_id,new_leader_id,vip,vip_end) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public void addClanMember(L2PcInstance player)
	{
        ...
        GabUtils.verifyClanCloak(player);
	}
	public void addClanMember(L2PcInstance player, int pledge)
	{
		final L2ClanMember member = new L2ClanMember(this, player);
		member.setPlayerInstance(player);
		member.setPledgeType(pledge);
		addClanMember(member);
		...
		switch (pledgeType)
		{
			case 0:
				switch (getLevel())
				{
					...
					case 0:
						return 10;
					default:
						return GabConfig.MAX_PLAYER_MAIN_CLAN;
				}
			case -1:
				return GabConfig.MAX_PLAYER_ACADEMY;
			case 100:
			case 200:
				switch (getLevel())
				{
					case 11:
						return GabConfig.MAX_PLAYER_ROYAL_ABOVE11;
					default:
						return GabConfig.MAX_PLAYER_ROYAL;
				}
			case 1001:
			case 1002:
			case 2001:
			case 2002:
				switch (getLevel())
				{
					...
					case 10:
					case 11:
						return GabConfig.MAX_PLAYER_KNIGHT_ABOVE9;
					default:
						return GabConfig.MAX_PLAYER_KNIGHT;
				}
			default:
				return 0;
		}
	...
	public void store()
	{
			...
			PreparedStatement ps = con.prepareStatement(INSERT_CLAN_DATA))
		{
            ...
            ps.setInt(14, isVip()? 1 : 0);
            ps.setLong(15, getVipEndTime());
			ps.execute();
			if (Config.DEBUG)
			{
			...
			PreparedStatement statement = con.prepareStatement(SELECT_CLAN_DATA))
		{
			...
			try (ResultSet clanData = statement.executeQuery())
			{
				if (clanData.next())
				{
                    ...
                    // Vip System
                    setVip(clanData.getInt("vip") == 1);
                    setVipEndTime(clanData.getLong("vip_end"));
	...
	public SubPledge createSubPledge(L2PcInstance player, int pledgeType, int leaderId, String subPledgeName)
	{
        ...
        if (!GabConfig.ALLOW_CREATE_SUBPLEDGE) {
            player.sendMessage("Royal and Knight creation has been disabled by the Administrator.");
            return null;
        }
		if (pledgeType == 0)
		{
	...
	public boolean checkClanJoinCondition(L2PcInstance activeChar, L2PcInstance target, int pledgeType)
	{
        ...
        if (OlympiadManager.getInstance().isRegistered(target) || target.isInOlympiadMode() || target.is_waitTimeTeleporterOly()) {
            activeChar.sendMessage("Target is currently registred in the olympiads. Try later");
            return false;
        }
    ...
    public void updateClanVIPInDB() {
        Connection con = null;
        PreparedStatement statement = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection();
            statement = con.prepareStatement("UPDATE clan_data SET vip=?,vip_end=? WHERE clan_id=?");
            statement.setInt(1, isVip()? 1 : 0);
            statement.setLong(2, getVipEndTime());
            statement.setInt(3, getId());
            statement.execute();
            if (Config.DEBUG)
                _log.info("New clan leader saved in db: " + getId());
        } catch (Exception e) {
            _log.warn( "Error saving clan: " + e.getMessage());
        } finally {
            DbUtils.close(con);
        }
    }
    ...
    public void setEndTime(String process, int val)
    {
        if (val > 0)
        {
            ...
            java.sql.Date dt = new Date(end_day);
            System.out.println("" + process + " end time for clan " + getName() + " is " + dt);
        }
        else
        {
            if (process.equals("vip"))
    ...
    private void restoreClanRecruitment()
    {
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            ...
            try (PreparedStatement statement = con.prepareStatement("SELECT * FROM clan_requiements where clan_id=" + getId());
            ...
            try (PreparedStatement statement = con.prepareStatement("SELECT * FROM clan_petitions where clan_id=" + getId());
                 ResultSet rset = statement.executeQuery())
            {
                while (rset.next())
                {
    ...
    public void updateRecrutationData()
    {
        try (Connection con = L2DatabaseFactory.getInstance().getConnection())
        {
            try (PreparedStatement statement = con.prepareStatement("INSERT INTO clan_requiements VALUES(" + getId() + ",0,'','','','','','','','','') ON DUPLICATE KEY UPDATE recruting=?,classes=?,question1=?,question2=?,question3=?,question4=?,question5=?,question6=?,question7=?,question8=?"))
            {
            ...
            try (PreparedStatement statement = con.prepareStatement("DELETE FROM clan_petitions WHERE clan_id=" + getId()))
            {
                statement.execute();
            }
    ...
    public synchronized boolean addPetition(int senderId, String[] answers, String comment)
    {
        ...
        if (getLeader().getPlayerInstance() != null)
            getLeader().getPlayerInstance().sendMessage("New Clan Petition has arrived!");
        return true;
    }
    ...
    {
        int max = 0;
        for (SubPledge unit : getAllSubPledges()){
            max =+ getMaxNrOfMembers(unit.getId());
        }

        return max == getMembersCount();
    }
}
