...
public class L2Party extends AbstractPlayerGroup
{
	...
	private int _itemLastLootEvent = 0;
	...
	private L2PcInstance getCheckedNextLooter(int ItemId, L2Character target) {
		return getCheckedNextLooter(ItemId, target, false);
	}

	private L2PcInstance getCheckedNextLooter(int ItemId, L2Character target, boolean event) {
		for (int i = 0; i < getMemberCount(); i++) {
			if (event ? (++_itemLastLootEvent >= getMemberCount()) : (++_itemLastLoot >= getMemberCount())) {
				if(event)
					_itemLastLootEvent = 0;
				else
					_itemLastLoot = 0;
			}
			...
			L2PcInstance member;
			try {
				member = getMembers().get(event? _itemLastLootEvent : _itemLastLoot);
				if (member.getInventory().validateCapacityByItemId(ItemId) && Util.checkIfInRange(Config.ALT_PARTY_RANGE2, target, member, true)) {
					return member;
				}
			} catch (Exception e) {
	...
	public void removePartyMember(L2PcInstance player, MessageType type)
	{
		if (getMembers().contains(player))
		{
            ...
            ChallengerZoneManager.getInstance().handlePartyLeave(player);
			...
			for (Entry<L2PcInstance, AtomicLong> member : toReward.entrySet())
			{
				if (member.getValue().get() > 0)
				{
					if (member.getKey().isPremium() && mustMultiply)
					{
						...
						member.getKey().addItem("Party", itemId, tempCount, target, true);
					}
					else
					{
						member.getKey().addItem("Party", itemId, member.getValue().get(), target, true);
					}
				}
			}
		}
    ...
    public int getHeallers() {
        Healers = 0;
        getClassPartyMembersHeallers();
        return Healers;
    }

    public int getDominator() {
        Dominators = 0;
        getClassPartyMembersDominator();
        return Dominators;
    }

    public int getTanks() {
        Tanks = 0;
        getClassPartyMembersTanks();
        return Tanks;
    }

    public int getBishops() {
        bishops = 0;
        getClassPartyMembersBishops();
        return bishops;
    }

    public void getClassPartyMembersHeallers() {
        for (L2PcInstance member : getMembers()) {
            if (((Arrays.stream(GabConfig.HEALLERS).anyMatch(i -> i == member.getClassId().getId())))) {
                Healers++;
            }
        }
    }
    public void getClassPartyMembersTanks() {
        for (L2PcInstance member : getMembers()) {
            if (((Arrays.stream(GabConfig.TANKS).anyMatch(i -> i == member.getClassId().getId())))) {
                Tanks++;
            }
        }
    }

    public void getClassPartyMembersDominator() {
        for (L2PcInstance member : getMembers()) {
            if (member.getClassId().getId() == 115 || member.getClassId().getId() == 51) {
                Dominators++;
            }
        }
    }
    public void getClassPartyMembersBishops() {
        for (L2PcInstance member : getMembers()) {
            if (((Arrays.stream(GabUtils.BISHOPS).anyMatch(i -> i == member.getClassId().getId())))) {
                bishops++;
            }
        }
    }

	public L2PcInstance getActualLooterForEvents(L2PcInstance player, int ItemId, boolean spoil, L2Character target) {
		L2PcInstance looter = player;
		...
		switch (_distributionType) {
			...
			case BY_TURN:
				if (!spoil) {
					looter = getCheckedNextLooter(ItemId, target, true);
				}
				break;
			case BY_TURN_INCLUDING_SPOIL:
				looter = getCheckedNextLooter(ItemId, target, true);
