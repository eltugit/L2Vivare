	...
	public void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (target.getObjectId() != getOwner().getObjectId())
		{
			...
			else
			{
                ...
                }else if ((GabConfig.OLY_VISUAL_RESTRICTION) && (target instanceof L2PcInstance) && ((L2PcInstance) target).isInOlympiadMode()) {
                    sm.addNpcName(this);
				...
				sm.addInt(damage);
                if(target instanceof L2PcInstance && this.getActingPlayer() != null){
                    this.getActingPlayer().getMuseumPlayer().addData("damage_given", damage);
	...
	public void reduceCurrentHp(double damage, L2Character attacker, L2Skill skill)
	{
		...
		if ((getOwner() != null) && (attacker != null))
		{
            ...
            if (attacker instanceof L2PcInstance) {
                if ((GabUtils.isInPvPInstance(this) && GabUtils.isInPvPInstance(attacker)) && PvPZoneManager.isAnnonym()) {
                    sm.addNpcName(this);
                    sm.addCharName(ConfigPvPInstance.ANONYM_STRING);
                }else if ((GabConfig.OLY_VISUAL_RESTRICTION) && ((L2PcInstance) attacker).isInOlympiadMode()) {
                    sm.addNpcName(this);
                    sm.addCharName(((L2PcInstance) attacker).getClassId().name().toUpperCase());
                }else {
                    sm.addNpcName(this);
