...
public class L2DoorInstance extends L2Character implements GeoCollision
{
    ...
    {
        if(attacker.isPlayable() && CSKOTHEvent.canAttackDoor(attacker))
            return true;
        if(getInstanceId() == GabConfig.ER_EVENT_INSTANCE_ID || getInstanceId() == GabConfig.EXTREME_EVENT_INSTANCE_ID || getInstanceId() == ConfigPvPInstance.PVP_INSTANCE_INSTANCE_ID)
            return false;
        return _isAttackableDoor;
    }

    public boolean getIsAttackableDoor(L2Character attacker)
    {
        if(attacker.isPlayable() && CSKOTHEvent.canAttackDoor(attacker))
            return true;
        if(getInstanceId() == GabConfig.ER_EVENT_INSTANCE_ID || getInstanceId() == GabConfig.EXTREME_EVENT_INSTANCE_ID || getInstanceId() == ConfigPvPInstance.PVP_INSTANCE_INSTANCE_ID)
    ...
    {
        if(this.getInstanceId() == CSKOTHEvent.getKothEventInstance())
            return true;
    ...
    public boolean isAutoAttackable(L2Character attacker)
    {
        ...
        if (getIsAttackableDoor(attacker))
        {
            return true;
        }
        if (!getIsShowHp())
