...
public class L2DoormenInstance extends L2NpcInstance
{
	...
	public void showChatWindow(L2PcInstance player)
	{
		...
		if (!isOwnerClan(player))
		{
			html.setFile(player, player.getHtmlPrefix(), "data/html/doormen/" + getTemplate().getId() + "-no.htm");
		}
		else
		{
			html.setFile(player, player.getHtmlPrefix(), "data/html/doormen/" + getTemplate().getId() + ".htm");
		}
	...
	protected void cannotManageDoors(L2PcInstance player)
	{
		...
		NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		html.setFile(player, player.getHtmlPrefix(), "data/html/doormen/" + getTemplate().getId() + "-busy.htm");
		player.sendPacket(html);
	}
	...
	protected void doTeleport(L2PcInstance player, String command)
	{
		...
		if (list != null)
		{
			...
			{
				player.teleToLocation(list.getX(), list.getY(), list.getZ(), player.getHeading(), player.getInstanceId(), 0);
			}
		}
		else
		{
