...
import gabriel.events.castleSiegeKoth.CSKOTHPcRewarder;
...
import l2r.gameserver.model.actor.instance.PcInstance.L2PcTeleportBook;
import l2r.gameserver.model.actor.instance.PcInstance.PcAdmin;
import l2r.gameserver.model.actor.instance.PcInstance.PcNevitSystem;
...
import gabriel.Utils.HeavyLightHelper;
...
import gabriel.listener.model.PlayerListenerList;
import gabriel.others.GiveClassCloak;
import gabriel.others.LevelUpSkills;
import gabriel.pvpInstanceZone.PvPZoneManager;
import gabriel.pvpInstanceZone.ConfigPvPInstance.ConfigPvPInstance;
import gabriel.pvpInstanceZone.utils.PVPInstanceRewarder;
import gabriel.restrictions.GlobalGabrielRestrictions;
...
{
	public Map<Integer, Long> lastUsedPot = new ConcurrentHashMap<>();

	//Altura safe para usar = 350
	public static int SAFE_CAST_HEIGHT_GROUND_SKILL = 350;
	public long usedPremiumChat = 0;
	...
	public boolean _inGabAbstractEvent = false;

    public long _randomCraftTimer = 0L;
	...
	{
		// Equip or unEquip
		if (getInventory().getItemByObjectId(item.getObjectId()) == null)
		{
			return;
		}
		...
		if (isEquiped)
		{
			...
			if (item.getName().contains("Mount Bracelet"))
			{
				if (this.isMounted())
				{
					this.dismount();
	...
	public void setClassId(int Id)
	{
		...
		try
		{
			...
			if (getClassId().level() == 3)
			{
				if (GabConfig.AUTO_ENCHANT_SKILLS_3RD_JOB)
				{
					LevelUpSkills.levelUpSkills(this);
				}
				
				if (GabConfig.ALLOW_GIVE_CLASS_CLOAK)
				{
					GiveClassCloak.giveCloak(this);
				}
			}
	...
	public boolean checkDoCastConditions(L2Skill skill)
	{
		...
		if (skill.isHeroSkill() && isInTournament())
		{
			sendMessage("You cannot use hero skill while in tournament");
			return false;
		}
	...
	public void doAutoLoot(L2Attackable target, int itemId, long itemCount)
	{
		...
		boolean isStackable = ItemData.getInstance().getTemplate(itemId).isStackable();
		...
		else
		{
			...
			else
			{
				...
				{
					addItem("Loot", itemId, 1, target, true);
				}
			}
		}
	}
	...
	public void doPickupItem(L2Object object)
	{
		...
		else
		{
			...
			else if ((target.getId() == Inventory.ADENA_ID) && (getInventory().getAdenaInstance() != null))
			{
				...
				boolean extra = isDonation() || isPremium();
				...
				addAdena("Pickup", target.fromMob && extra ? tempCount : target.getCount(), null, true);
			...
			else
			{
				...
				{
					addItem("Pickup", target.getId(), target.fromMob && extra ? tempCount : target.getCount(), null, true);
				}
...
//		this.spawnMe();
		...
		if (isInOlympiadMode() && (olyzone != null) && (olyEnemy != null))
		{
			broadcastPacket(new ExOlympiadUserInfo(this));
			broadcastPacket(new ExOlympiadUserInfo(olyEnemy));
			olyzone.broadcastPacket(new ExOlympiadUserInfo(this));
			olyzone.broadcastPacket(new ExOlympiadUserInfo(olyEnemy));
		}
	}
	
	public L2OlympiadStadiumZone olyzone = null;
	public L2PcInstance olyEnemy = null;
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (GabUtils.isInPvPInstance(this))
			{
				PVPInstanceRewarder.getInstance().onKillPvPInstance(this, pk);
				PVPInstanceRewarder.getInstance().onDiePvPInstance(this, pk);
			}

			if (GabConfig.CSKOTH_EVENT_ENABLED && CSKOTHEvent.isStarted())
				CSKOTHEvent.onKill(killer, this);
			...
			if (pk != null)
			{
				...
				if (KrateiCubeManager.getInstance().isInProgress(getQuickVarI(QuickVarType.KRATEI_CUBE_LVL.getCommand(), -1)) && KrateiCubeManager.getInstance().isKrateiParticipant(this))
				{
					KrateiCubeManager.getInstance().onPlayerKill(killer, this);
				}
			...
			else
			{
				...
				if ((pk == null) || !pk.isCursedWeaponEquipped())
				{
					...
					boolean noExpLoss = (KrateiCubeManager.getInstance().isInProgress(getQuickVarI(QuickVarType.KRATEI_CUBE_LVL.getCommand(), -1)) && KrateiCubeManager.getInstance().isKrateiParticipant(this));
					if (!isLucky() && (insideSiegeZone || !insidePvpZone) && !getNevitSystem().isAdventBlessingActive() && !noExpLoss)
					{
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		if (_inGabAbstractEvent && GabConfig.CSKOTH_ARENA_KILL_ENABLE) {
			increasePvpKills(target, true);
			return;
		}
		if (isInsideZone(ZoneIdType.SIEGE) && targetPlayer.isInsideZone(ZoneIdType.SIEGE) && target.isPlayer())
		{
			SiegeRankManager.getInstance().increaseKills(this);
			SiegeRankManager.getInstance().increaseDeaths(targetPlayer);
			if (GabConfig.ENABLE_PVP_COUNT_SIEGETW)
			{
				increasePvpKills(targetPlayer, true);
				return;
			}
		...
		if (getInstanceId() == GabConfig.ER_EVENT_INSTANCE_ID)
		{
            EpicRaidRankManager.getInstance().increaseKills(this);
            EpicRaidRankManager.getInstance().increaseDeaths(targetPlayer);
			increasePvpKills(target);
			return;
		}
		
		if ((getInstanceId() == GabConfig.PARTY_AREA_INSTANCE_ID) && GabConfig.PARTY_AREA_EVENT_PVP_INSIDE)
		{
			increasePvpKills(target);
			return;
		}
		
		if ((getInstanceId() == GabConfig.EXTREME_EVENT_INSTANCE_ID) && GabConfig.EXTREME_EVENT_PVP_INSIDE)
		{
			increasePvpKills(target);
			return;
		}
		...
		// enchant random equipped item on char kill
		// enchantRandomEquippedItem(this);
	...
	public boolean canAnnounce()
	{
		...
		if (GabUtils.isInPvPInstance(this) && PvPZoneManager.isAnnonym())
		{
			return false;
		}
		return true;
	...
	public void increasePvpKills(L2Character target, boolean event)
	{
		if (target instanceof L2PcInstance)
		{
			if (canAnnounce())
			{
				sendPacket(new CreatureSay(1, 15, "[" + GabConfig.L2GABSON_SERVER_NAME + "]", getName() + " Defeated " + target.getName() + " !"));
			}

			if (event || AntiFeedManager.getInstance().check(this, target))
			{
				if (CustomServerConfigs.PVP_SPREE_SYSTEM) // tem q tar on
				{
					spreeKills++;
				...
				if (PvpRewardSystemConfigs.ENABLE_PVP_REWARD_SYSTEM)
				{
					...
					if (target.isPlayer() && !GabUtils.isInPvPInstance(this))
					{
						PvpRewardHandler.pvpRewardSystem(this, target.getActingPlayer());
					}
				}
				CSKOTHPcRewarder.reward(this);
				if (GabConfig.WEEKLYRANK_ALLOW) {
					if (GabConfig.WEEKLYRANK_ALLOW_CLAN)
						WeeklyManager.getInstance().increaseClanKill(this);
					if (GabConfig.WEEKLYRANK_ALLOW_PLAYER)
						WeeklyManager.getInstance().increasePlayerKill(this);
				}
				// Color system
	...
	{
		if (SunriseEvents.isInEvent(this) || isInGabEvents())
		{
			return;
		}
		if(_inGabAbstractEvent)
			return;
	...
	// Certification CBB
	public final Map<Integer, CertiData> _certi_data = new HashMap<>();
	/*
	 * index emergent1 emergent2 emergent3 emergent4 skill1 skill2 skill3 transform1 transform2 transform3
	 */
	private static final String SELECT_CERTI = "SELECT * FROM character_certification WHERE object_id=?";
	
	public void loadCerti()
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			try (PreparedStatement ps = con.prepareStatement(SELECT_CERTI))
			{
				...
				try (ResultSet rs = ps.executeQuery())
				{
					while (rs.next())
					{
						...
						//
						CertiData data = new CertiData(this, index);
						//
						data._emergent[0] = rs.getInt("emergent1");
						data._emergent[1] = rs.getInt("emergent2");
						data._emergent[2] = rs.getInt("emergent3");
						data._emergent[3] = rs.getInt("emergent4");
						//
						data._skill[0] = rs.getInt("skill1");
						data._skill[1] = rs.getInt("skill2");
						data._skill[2] = rs.getInt("skill3");
						//
						data._transform[0] = rs.getInt("transform1");
						data._transform[1] = rs.getInt("transform2");
						data._transform[2] = rs.getInt("transform3");
						//
						_certi_data.put(index, data);
					}
				}
			}
		}
		...
		{
			_log.error("Failed loadCerti");
			e.printStackTrace();
		}
		getCertiData().updateEffects();
	}
	
	private static final String INSERT_CERTI = "INSERT INTO character_certification VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE emergent1=?,emergent2=?,emergent3=?,emergent4=?,skill1=?,skill2=?,skill3=?,transform1=?,transform2=?,transform3=?";
	
	public void insertCerti()
	{
		if (_certi_data.size() > 0)
		{
			try (Connection con = L2DatabaseFactory.getInstance().getConnection())
			{
				try (PreparedStatement ps = con.prepareStatement(INSERT_CERTI))
				{
					for (CertiData info : _certi_data.values())
					{
						ps.setInt(1, getObjectId());
						ps.setInt(2, info.getIndex());
						//
						ps.setInt(3, info.getEmergent1());
						ps.setInt(4, info.getEmergent2());
						ps.setInt(5, info.getEmergent3());
						ps.setInt(6, info.getEmergent4());
						ps.setInt(7, info.getSkill1());
						ps.setInt(8, info.getSkill2());
						ps.setInt(9, info.getSkill3());
						ps.setInt(10, info.getTransform1());
						ps.setInt(11, info.getTransform2());
						ps.setInt(12, info.getTransform3());
						//
						ps.setInt(13, info.getEmergent1());
						ps.setInt(14, info.getEmergent2());
						ps.setInt(15, info.getEmergent3());
						ps.setInt(16, info.getEmergent4());
						ps.setInt(17, info.getSkill1());
						ps.setInt(18, info.getSkill2());
						ps.setInt(19, info.getSkill3());
						ps.setInt(20, info.getTransform1());
						ps.setInt(21, info.getTransform2());
						ps.setInt(22, info.getTransform3());
						ps.addBatch();
					}
					ps.executeBatch();
				}
			...
			{
				_log.error("Failed insertCerti");
				e.printStackTrace();
			}
		}
	}
	
	private static final String DELETE_CERTI = "DELETE FROM `character_certification` WHERE `object_id`=? AND `index`=?";
	...
	public void deleteCerti(int index)
	{
		...
		{
			_log.error("Failed deleteCerti");
			e.printStackTrace();
		}
	}
	
	public CertiData getCertiData()
	{
		int index = isSubClassActive() ? getClassIndex() : 0;
		if (!_certi_data.containsKey(index))
		{
			_certi_data.put(index, new CertiData(this, index));
		}
		return _certi_data.get(index);
	}
	...
	public static L2PcInstance getOfflinePlayerData(int objectId)
	{
		return restore(objectId);
	}
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				if (rset.next())
				{
					...
					player.getBookmarkEx().setBookMarkSlot(rset.getInt("BookmarkSlot"));
	...
	private void storeCharBase()
	{
			...
			PreparedStatement statement = con.prepareStatement(UPDATE_CHARACTER))
		{
			...
			statement.setInt(47, getBookmarkEx().getBookMarkSlot());
			...
			// When closing character, we must add the online value to the stats for it to be saved
			
			statement.execute();
			
			// The same for the longest online time, we must calculate it on exit
	...
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		{
//			if (isInTournament() && ((L2PcInstance) attacker).isInTournament())
//			{
//				return true;
//			}
			if (GlobalGabrielRestrictions.getInstance().isAutoAttackable((L2PcInstance) attacker, this))
			{
				return true;
			}
		}
...
//		if (CSKOTHEvent.isStarted() && CSKOTHEvent.isPlayerParticipant(getObjectId()))
//			return true;
		...
		else if (attacker instanceof L2DefenderInstance)
		{
			if (getClan() != null)
			{
				...
				// TODO GABRIEL CHECK ATTACK DEFENDER?
	...
	{
		if (GabConfig.DEBUG_GABS && (this instanceof L2PcInstance))
		{
			System.out.println("GABS: useMagic: " + this.getName());
		}
		
		if (GlobalGabrielRestrictions.getInstance().useMagic(this))
		{
			forceUse = true;
		}
...
//		}
		...
		// If a skill is currently being used or attacking, queue this one.
		// Attacking now check exists in order to allow hit to land if player tries to use skill before attack is finished
	...
	public boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove, boolean simultaneous)
	{
		// ************************************* Check Player State *******************************************
		if (GabConfig.DEBUG_GABS && (this instanceof L2PcInstance))
		{
			System.out.println("GABS: checkUseMagicConditions: " + this.getName());
		}
		
		// TW cast!
		// if(TerritoryWarManager.getInstance().isTWInProgress() && skill.getId() == 847 && getActingPlayer().isCombatFlagEquipped() && !WardLimiter.canSummonWard(getActingPlayer())) {
		// getActingPlayer().sendMessage("Your clan has the maximum amount of wards! You can't summon one more!");
		// if (GabConfig.DROP_ON_MAX_WARDS) {
		// L2ItemInstance item = getActingPlayer().getInventory().getPaperdollItem(Inventory.PAPERDOLL_RHAND);
		// TerritoryWarManager.getInstance().dropCombatFlag(getActingPlayer(), false, false);
		// getActingPlayer().sendMessage("Your clan already have the max amount of Territory Wards! You cannot summon one more!");
		// getActingPlayer().destroyItem("CombatFlag", item, null, true);
		// InventoryUpdate iu = new InventoryUpdate();
		// iu.addRemovedItem(item);
		// iu.addModifiedItem(item);
		// getActingPlayer().sendPacket(iu);
		// getActingPlayer().broadcastUserInfo();
		// }
		// return false;
		// }
		if (GlobalGabrielRestrictions.getInstance().checkUseMagicConditionsForce(this))
		{
			forceUse = true;
		}
...
//		}
		...
		if (isSitting() && !skill.isSimultaneousCast() && !simultaneous)
		{
		...
		if (target.isDoor())
		{
			if(CSKOTHEvent.canAttackDoor(this))
				return true;
		...
		// check if this is a Player, then check if target is a raid or grand boss, and checks also if the skill is a heal or heal percent. if all true, fail cast
		if (!GabConfig.PLAYERS_CAN_HEAL_RB && (this instanceof L2PcInstance) && !this.isGM() && ((target instanceof L2RaidBossInstance) || (target instanceof L2GrandBossInstance)
		// || (target.is) || (target.isRaid()) || (target.isRaidBoss())
		) && (skill.hasEffectType(L2EffectType.HEAL) || skill.hasEffectType(L2EffectType.HEAL_PERCENT) || skill.hasEffectType(L2EffectType.HEAL_OVER_TIME)))
		{
		...
		if (skill.isOffensive())
		{
			...
			if (GlobalGabrielRestrictions.getInstance().checkUseMagicConditions(this, target.getActingPlayer()))
			{
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
			...
			if (forceUse && !canAttackSiegier(this, target))
			{
				switch (sklTargetType)
				{
					case AURA:
		...
		if (skill.getCastRange() > 0)
		{
			if (sklTargetType == L2TargetType.GROUND)
			{
				if (!l2r.geoserver.GeoData.getInstance().canSeeTarget(this, worldPosition)) {
					if(Math.abs(getZ() - worldPosition.getZ()) > SAFE_CAST_HEIGHT_GROUND_SKILL || getDistance(worldPosition) > skill.getCastRange()) {
			...
			else if (!GeoData.getInstance().canSeeTarget(this, target) && ((skill.getTargetType() != L2TargetType.HOLY) && (skill.getTargetType() != L2TargetType.FLAGPOLE)))
			{
				sendPacket(SystemMessageId.CANT_SEE_TARGET);
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
		...
		if ((skill.getFlyType() == FlyType.CHARGE) && (Config.GEODATA) && !GeoData.getInstance().canMove(this, target))
		{
			sendPacket(SystemMessageId.THE_TARGET_IS_LOCATED_WHERE_YOU_CANNOT_CHARGE);
			return false;
		}
		
		if (((skill.getFlyType() == FlyType.DUMMY) || skill.hasEffectType(L2EffectType.TELEPORT_TO_TARGET)) && (Config.GEODATA) && !GeoData.getInstance().canMove(this, target))
		{
			...
			if ((targetPlayer != this) && !(isInDuel() && (targetPlayer.getDuelId() == getDuelId())) && !isInsideZone(ZoneIdType.PVP) && !targetPlayer.isInsideZone(ZoneIdType.PVP))
			{
				...
				if (((skilldat != null) && !skilldat.isCtrlPressed() && skill.isOffensive() && !srcIsSummon) || ((skilldatpet != null) && !skilldatpet.isCtrlPressed() && skill.isOffensive() && srcIsSummon))
				{
					if ((getClan() != null) && (targetPlayer.getClan() != null))
					{
						...
						if (isInTournament() && ((L2PcInstance) target).isInTournament())
						{
							return true;
						}
						if(_inGabAbstractEvent)
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		try
		{
			if (isPendingRevive() || isProcessingRequest())
			{
				return false;
			}
			...
			if (GabConfig.CLASS_ID_NERF_HEAVY.stream().anyMatch(e -> e == this.getClassId().getId()))
			{
				L2Skill skill = SkillData.getInstance().getSkill(GabConfig.CLASS_HEAVY_NERF_SKILL, 1);
				if (skill == null)
				{
					System.out.println("Skill with ID: " + GabConfig.CLASS_HEAVY_NERF_SKILL + " dont exist!");
				}
				else
				{
			...
			if (GabConfig.CLASS_ID_NERF_BOW_USAGE.stream().anyMatch(e -> e == this.getClassId().getId()))
			{
				L2Skill skill = SkillData.getInstance().getSkill(GabConfig.BOW_USAGE_NERF_SKILL, 1);
				if (skill == null)
				{
					System.out.println("Skill with ID: " + GabConfig.CLASS_ID_NERF_BOW_USAGE + " dont exist!");
				}
				else
				{
					this.addSkill(skill, false);
			...
			if (!GabConfig.ALLOW_LIGHT_USE_HEAVY)
			{
				HeavyLightHelper.handleLightHeavyClassChange(this);
			}
			if (!GabConfig.ALLOW_HEAVY_USE_LIGHT)
			{
				HeavyLightHelper.handleHeavyLightClassChange(this);
			}
	...
	{
		final L2WaterZone waterZone = ZoneManager.getInstance().getZone(this, L2WaterZone.class);
		if (isInsideZone(ZoneIdType.WATER) && (waterZone != null) && (Math.abs(Math.abs(waterZone.getWaterZ()) - Math.abs(getZ())) > 16))
		{
			startWaterTask();
		}
		else
	...
	public void onPlayerEnter()
	{
		...
		// TODO GABRIEL Configurable Master Name per Access Level
	...
	public List<L2Effect> _togglesToReUse = new LinkedList<>();

	public long canBuff = 0L;
	...
	public void doRevive()
	{
		...
		canBuff = System.currentTimeMillis() + (5*1000L);
	...
	public final void onTeleported()
	{
		...
		if (Config.SKILL_CHECK_ENABLE && !Config.ALT_GAME_SKILL_LEARN)
		{
			checkAllowedSkillsRunning();
		}
		// Force a revalidation
		revalidateZone(true);
		
		setLastClientPosition(getLocation());
		setLastServerPosition(getLocation());
		...
		if ((summon != null) && !isMounted())
		{
			...
			summon.updateAndBroadcastStatus(BroadcastStatusType.TELEPORTED);
		}
		CSKOTHEvent.onTeleported(this);
	}
	...
	public synchronized void cleanup()
	{
		...
		if (isInTournament())
		{
			TTournamentManager.getInstance().onDisconnect(this);
		}
		...
		{
			PvPZoneManager.getInstance();
			PvPZoneManager.handleLeavePvPZone(this, true);
			if (isInArenaEvent())
			{
				setTeam(Team.NONE);
			}
			setInArenaEvent(false);
			// System.out.println("Quitted the game "+PvPZoneManager.getInstance().playerinInstance);
		}
		catch (Exception e)
		{
			e.printStackTrace();
		...
		try
		{
			if (KrateiCubeManager.getInstance().isKrateiParticipant(this))
			{
				KrateiCubeManager.getInstance().onDisconnect(this);
			}
		}
		catch (Exception e)
		{
	...
	public void calculateDeathPenaltyBuffLevel(L2Character killer)
	{
		...
		if (isResurrectSpecialAffected() || (CSKOTHEvent.isStarted() && CSKOTHEvent.isPlayerParticipant(getObjectId())) || isLucky() || isInsideZone(ZoneIdType.PVP) || isInsideZone(ZoneIdType.SIEGE) || canOverrideCond(PcCondOverride.DEATH_PENALTY))
		{
			return;
		}
		double percent = 1.0;
	...
	public final void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (miss)
		{
			if (target instanceof L2PcInstance)
			{
				...
				PvPZoneManager.getInstance();
				if ((GabUtils.isInPvPInstance(this) && GabUtils.isInPvPInstance(target)) && PvPZoneManager.isAnnonym())
				{
					sm.addPcName(ConfigPvPInstance.ANONYM_STRING);
					sm.addCharName(ConfigPvPInstance.ANONYM_STRING);
				}
				else if (GabConfig.OLY_VISUAL_RESTRICTION && this.isInOlympiadMode() && ((L2PcInstance) target).isInOlympiadMode())
				{
					sm.addPcName(((L2PcInstance) target).getClassId().name().toUpperCase());
					sm.addCharName(this.getClassId().name().toUpperCase());
				}
				else
				{
			...
			else if (GabConfig.OLY_VISUAL_RESTRICTION && (target instanceof L2PcInstance) && this.isInOlympiadMode() && ((L2PcInstance) target).isInOlympiadMode())
			{
				sm.addPcName(this.getClassId().name().toUpperCase());
			}
			else
			{
				sm.addPcName(this);
		...
		else
		{
			...
			{
				sm.addPcName(this.getClassId().name().toUpperCase());
				sm.addCharName(((L2PcInstance) target).getClassId().name().toUpperCase());
			}
			else
			{
				sm.addPcName(this);
		...
		if (target instanceof L2PcInstance)
		{
			getMuseumPlayer().addData("damage_given", damage);
			target.getActingPlayer().getMuseumPlayer().addData("damage_received", damage);
	...
	{
		if (!Config.ENABLE_FALLING_DAMAGE || isDead() || isFlying() || isFlyingMounted() || isInBoat())
		{
			return;
		}
		_lastFalling = System.currentTimeMillis();
		
		final int damage = (int) Formulas.calcFallDam(this, height);
		if (damage > 0)
		{
	...
	public void setFarmBot(boolean farmBot)
	{
		if (farmBot)
		{
			ThreadPoolManager.getInstance().scheduleGeneral(() ->
			{
				stopAbnormalEffect(AbnormalEffect.REAL_TARGET);
				setIsInvul(false);
				setIsParalyzed(false);
				if (hasSummon())
				{
					getSummon().setIsParalyzed(false);
					getSummon().setIsInvul(false);
				}
	...
	public boolean isFriend(L2PcInstance target)
	{
		...
		if (GabUtils.isInPvPInstance(this))
		{
			return GlobalGabrielRestrictions.getInstance().isFriend(this, target);
		}

		if (GlobalGabrielRestrictions.getInstance().isFriend(this, target)) {
			return true;
		}
		...
		if (isInsideSiegeZone && isInSiege() && (getSiegeState() != 0) && (target.getSiegeState() != 0))
		{
			...
			if ((siege != null) && siege.isAttackersInAlly())
			{
	...
	public boolean isEnemy(L2PcInstance attacker)
	{
		...
		if(CSKOTHEvent.isPlayerParticipant(this.getObjectId())) {
			return !GlobalGabrielRestrictions.getInstance().isFriend(this, attacker);
		}
		
		// Check if the attacker is in Kratei Cube game
		if (KrateiCubeManager.getInstance().isKrateiParticipant(attacker) && KrateiCubeManager.getInstance().isKrateiParticipant(this) && KrateiCubeManager.getInstance().isInProgress(attacker.getQuickVarI(QuickVarType.KRATEI_CUBE_LVL.getCommand(), -1)))
		{
			return true;
		}
	...
	private final PcNevitSystem _nevitSystem = new PcNevitSystem(this);
	
	public PcNevitSystem getNevitSystem()
	{
		return _nevitSystem;
	}
	...
	private final L2PcTeleportBook _bookmarkExtension = new L2PcTeleportBook(this);
	...
	/**
	 * Returns the Extension class for all bookmark related functions
	 * @return
	 */
	public L2PcTeleportBook getBookmarkEx()
	{
		return _bookmarkExtension;
	}
	...
	private boolean _donationService = false;
	
	public void setDonationService(boolean donationService)
	{
	...
	private int arenaQuantity = 0;
	
	public int getArenbaQuantity()
	{
		return arenaQuantity;
	}
	
	public void setArenaQuantity(int arenaQuantity)
	{
		this.arenaQuantity = arenaQuantity;
	}
	
	public void healToMaxPvPInstance()
	{
		setCurrentCp(getMaxCp());
		setCurrentHp(getMaxHp());
		setCurrentMp(getMaxMp());
	...
	public void enterObserverTournamentMode(Location loc, int instanceId, int quantity)
	{
		if (getSummon() != null)
		{
			getSummon().unSummon(this);
		...
		{
			for (CubicInstance cubic : this.getCubics().values())
			{
				cubic.deactivate();
			}
		...
		if (getParty() != null)
		{
			getParty().removePartyMember(this, MessageType.Left);
		}
	...
	private ScheduledFuture<?> colorTask = null;
	public void startTryColorTask(){
		if(colorTask != null){
			colorTask.cancel(true);
			colorTask = null;
		}
		colorTask = ThreadPoolManager.getInstance().scheduleGeneral(() -> {
			sendPacket(SystemMessageId.NO_LONGER_TRYING_ON);
			deleteQuickVar("tryColor");
	...
	public void leaveTournamentObserverMode()
	{
		...
		{
			try{
				TTournamentManager.getInstance().getFight(arenaQuantity, getInstanceId()).getToRemoveSpectators().add(this);
			}catch (Exception e){
				try{
					LOLTournamentManager.getInstance().getFight(arenaQuantity, getInstanceId()).getToRemoveSpectators().add(this);
				}catch (Exception ee){
				}
			}
			arenaQuantity = 0;
	...
	private int _team4t = 0;
	
	public void setTeam4t(int id)
	{
		if ((id >= 0) && (id <= 4))
		{
			_team4t = id;
	...
	public int getTeam4t()
	{
		return _team4t;
	}
	
	private boolean _waitTimeTeleporterOly = false;
	
	public boolean is_waitTimeTeleporterOly()
	{
		return _waitTimeTeleporterOly;
	}
	
	public void set_waitTimeTeleporterOly(boolean _waitTimeTeleporterOly)
	{
		this._waitTimeTeleporterOly = _waitTimeTeleporterOly;
	}
	
	public void checkAllowedSkillsRunning()
	{
		if (!isGM() && Config.SKILL_CHECK_ENABLE)
		{
			for (L2Skill allSkill : getAllSkills())
			{
				if ((!isGM() || Config.SKILL_CHECK_GM))
				{
					if (!SafeSkill.isSafeSkill(allSkill, this))
					{
						Util.handleIllegalPlayerAction(this, "Player " + this.getName() + " has invalid skill " + allSkill.getName() + " (" + allSkill.getId() + "/" + allSkill.getLevel() + "), class:" + ClassListData.getInstance().getClass(this.getClassId()).getClassName(), IllegalActionPunishmentType.BROADCAST);
						if (Config.SKILL_CHECK_REMOVE)
						{
							removeSkill(allSkill);
						}
	...
	private boolean insideCastleSiegeZone = false;
	
	public boolean isInsideCastleSiegeZone()
	{
		return insideCastleSiegeZone;
	}
	
	public void setInsideCastleSiegeZone(boolean insideCastleSiegeZone)
	{
		this.insideCastleSiegeZone = insideCastleSiegeZone;
	}
	...
	public PlayerListenerList getListenersGab()
	{
		if (listeners == null)
		{
			synchronized (this)
	...
	public Set<Integer> getAutoPot()
	{
		return _autoPotTasks.keySet();
	}
	
	private L2ItemInstance itemDlg = null;
	
	public L2ItemInstance getItemDlg()
	{
		return itemDlg;
	}
	
	public void setItemDlg(L2ItemInstance itemDlg)
	{
		this.itemDlg = itemDlg;
	}
	...
	{
		return CSKOTHEvent.isPlayerParticipant(this.getObjectId());
	}
	
	public boolean isInEvents()
	{
		return SunriseEvents.isInEvent(this) || SunriseEvents.isRegistered(this);
	}
	
	private ExServerPrimitive drawLines = null;
	
	public void setDrawLines(ExServerPrimitive packet)
	{
		drawLines = packet;
		sendPacket(drawLines);
	}
	
	public ExServerPrimitive getDrawLines()
	{
		return drawLines;
	}
	...
	public void ask(ConfirmDlg dlg, OnAnswerListener listener)
	{
		...
		_askDialog = new Pair<Integer, OnAnswerListener>(dlg.get_requesterId(), listener);
		sendPacket(dlg);
	}
	...
	private long lastMovementUsed;
	
	public long getLastMovementUsed()
	{
		return lastMovementUsed;
	}
	
	public void setLastMovementUsed(long lastMovementUsed)
	{
		this.lastMovementUsed = lastMovementUsed;
	}
	...
	{
		return isInEvents() || isInOfflineMode() || isInOlympiad() || isInOlympiadMode() || OlympiadManager.getInstance().isRegistered(this) || OlympiadManager.getInstance().isRegisteredInComp(this) || this.isInCombat();
	}
	
	public boolean isInsidePeaceZone()
	{
		return isInsideZone(ZoneIdType.TOWN) || isInsideZone(ZoneIdType.PEACE);
	}
	...
	final public boolean canBeKnockedBack(int objectid)
	{
		if (objectid != 0)
		{
			return (objectid == knockbackObjectId) || (GameTimeController.getInstance().getGameTicks() > knockbackProtectionTimerTick);
		}
		return GameTimeController.getInstance().getGameTicks() > knockbackProtectionTimerTick;
	}
	
	final public void setKnockbackProtectionTime(int objectid)
	{
		knockbackProtectionTimerTick = GameTimeController.getInstance().getGameTicks() + 150;
		knockbackObjectId = objectid;
	}
	
	protected ScheduledFuture<?> tryGab = null;
	
	public void cancelTryGab()
	{
		if (tryGab != null)
		{
			tryGab.cancel(true);
	...
	public void setTryGab(Runnable runnable)
	{
		if (tryGab != null)
		{
			tryGab.cancel(true);
		...
		this.tryGab = ThreadPoolManager.getInstance().scheduleGeneral(runnable, 6 * 1000);
	}
	
	protected ScheduledFuture<?> tryWeapon = null;
	
	public void cancelTryWeapon()
	{
		if (tryWeapon != null)
		{
			tryWeapon.cancel(true);
		}
	}
	
	public void setTryWeapon(L2ItemInstance weapon)
	{
		if (tryWeapon != null)
		{
			tryWeapon.cancel(true);
		...
		this.tryWeapon = ThreadPoolManager.getInstance().scheduleGeneral(new TryDressMeCustom(this, weapon), 6 * 1000);;
	}
	
	private final Map<Integer, L2Skill> temporarySkills = new LinkedHashMap<>();
	...
	public void deleteSubClass(int classIndex)
	{
		if (!_subclassLock.tryLock())
		{
			return;
	...
	public boolean hatesRace(L2Character target)
	{
		...
		final Race targetRace = target.getActingPlayer().getRace();
		
		switch (getRace())
		{
			case HUMAN:
				return targetRace == Race.KAMAEL;
			case ELF:
				return targetRace == Race.DARK_ELF;
			case DARK_ELF:
				return targetRace == Race.ELF;
			case ORC:
				return targetRace == Race.DWARF;
			case DWARF:
				return targetRace == Race.ORC;
			case KAMAEL:
				return targetRace == Race.HUMAN;
		}
		
		return false;
	}
	
	public long lastMessageGlobalSent = 0;
	public long lastMessageTradeSent = 0;
	...
	public void setIncorrectValidateCount(int i)
	{
		_incorrectValidateCount = i;
	}
	...
	public boolean isInGabEvents()
	{
		return GabUtils.isInPvPInstance(this) || GabUtils.isInEpicRaid(this) || GabUtils.isInPartyZone(this) || GabUtils.isInExtremeZone(this);
	}
	...
	public void clearDuppedBuffs()
	{
		int improved_movement = 1504;
		int improved_combat = 1499;
		int wind_walk = 1204;
		int shield = 1040;
		L2Effect imp_mov = null;
		L2Effect imp_c = null;
		L2Effect ww = null;
		L2Effect sh = null;
		
		for (L2Effect ef : getAllEffects())
		{
			if (ef.getSkill().getId() == improved_movement)
			{
				imp_mov = ef;
			}
			else if (ef.getSkill().getId() == improved_combat)
			{
				imp_c = ef;
			}
			else if (ef.getSkill().getId() == wind_walk)
			{
				ww = ef;
			}
			else if (ef.getSkill().getId() == shield)
			{
				sh = ef;
			}
			
			if (((imp_mov != null) && (ww != null)) || ((imp_c != null) && (sh != null)))
			{
				stopAllEffects();
			}
		}
	...
	/** Auto Potion System */
	public boolean hpPot = false;
	public boolean manaPot = false;
	public boolean cp1Pot = false;
	public boolean cp2Pot = false;
	public boolean kamaelPot = false;


	public List<L2Skill> getSharedSkills(){
		List<L2Skill> skills = new LinkedList<>();

		for (Entry<String, String> entry : getSunriseVariables().getVars().entrySet()) {
			String var = entry.getKey();
			if(!var.startsWith("sharedSkill"))
				continue;
	...
	public void addSharedSkills(){
		for (L2Skill sharedSkill : getSharedSkills()) {
			addSkill(sharedSkill, false);
		}
	}
}