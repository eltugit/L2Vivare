...
public class L2RaidBossInstance extends L2MonsterInstance
{
	...
	private boolean barakielKillNobles = true;
	...
	public boolean doDie(L2Character killer)
	{
        ...
        if (!isEventRaid())
		{
            if(barakielKillNobles && player != null && getId() == 25325){
                if(player.isInParty()){
                    for (L2PcInstance member : player.getParty().getMembers()) {
                        if(member.getLevel() > 75 && !member.isNoble()){
                            member.setNoble(true);
                            member.sendMessage("You have been rewarded with the nobless status for killing barakiel!");
                        }
                    }
                }else{
                    if(player.getLevel() > 75 && !player.isNoble()){
                        player.setNoble(true);
                        player.sendMessage("You have been rewarded with the nobless status for killing barakiel!");
                    }
                }
            }
	...
	protected void checkAndReturnToSpawn()
	{
		...
		if (!isInsideRadius(spawn.getX(), spawn.getY(), spawn.getZ(), Math.max(Config.MAX_DRIFT_RANGE, 2000), true, false))
		{
