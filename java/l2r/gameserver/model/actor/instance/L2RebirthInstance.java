...
public final class L2RebirthInstance extends L2NpcInstance {
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        if (command.startsWith("performRebirth")) {
            ...
            boolean levelOfPlayer = RebirthEngineConfigs.REBIRTH_LEVEL_MAX.containsKey(player.getLevel());
            if(levelOfPlayer){
                int maxRebirthLevel = RebirthEngineConfigs.REBIRTH_LEVEL_MAX.get(player.getLevel());
                if (currBirth >= maxRebirthLevel) {
            ...
            boolean levelToBeInList = RebirthEngineConfigs.REBIRTH_REQUIREMENTS_LEVEL.containsKey(player.getLevel());

            if (levelToBeInList) {
                int requiredLevel = RebirthEngineConfigs.REBIRTH_REQUIREMENTS_LEVEL.get(player.getLevel());
