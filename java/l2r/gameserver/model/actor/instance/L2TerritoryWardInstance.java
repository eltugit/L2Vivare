...
//            if (!WardLimiter.canSummonWard(killer.getActingPlayer())) {
//                sendMessage = false;
//            } else if (GabConfig.ONE_WARD_AT_TIME_CLAN && WardLimiter.clanHasWardAlready(killer.getActingPlayer())) {
//                sendMessage = false;
//            }
    ...
    private boolean canPickup(L2PcInstance player)
    {
        ...
        if (TerritoryWarManager.getInstance().getTotalWards(player.getSiegeSide() - 80) >= GabConfig.MAX_WARD_CLAN)
        {
            player.sendMessage("You can not take this flag, you can have max " + GabConfig.MAX_WARD_CLAN + " flags ");
            return false;
        }
    ...
    private boolean cantInteract(L2PcInstance player)
    {
        ...
        if (TerritoryWarManager.getInstance().getTotalWards(player.getSiegeSide() - 80) >= GabConfig.MAX_WARD_CLAN)
        {
            player.sendMessage("You can not interact with this flag, you can take max " + GabConfig.MAX_WARD_CLAN+ " flags ");
            player.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE);
            player.sendPacket(ActionFailed.STATIC_PACKET);
            return true;
        }
