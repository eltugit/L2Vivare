...
public class PcFunc
{
	...
	public static boolean checkSummonTargetStatus(L2PcInstance target, L2PcInstance activeChar)
	{
		...
		if (CSKOTHEvent.isPlayerParticipant(target.getObjectId()) && !CSKOTHEvent.onEscapeUse(target.getObjectId())) {
		...
		if (CSKOTHEvent.isPlayerParticipant(activeChar.getObjectId()) && !CSKOTHEvent.onEscapeUse(activeChar.getObjectId())) {
			activeChar.sendPacket(SystemMessageId.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING);
			return false;
		}
