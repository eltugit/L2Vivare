...
public class CharKnownList extends ObjectKnownList
{
	...
	public List<L2Character> getKnownCharactersById(int id)
	{
		...
		for (L2Object obj : getKnownObjects().values())
		{
			if ((obj instanceof L2Character) && (obj.getId() == id))
			{
				result.add((L2Character) obj);
			}
		}
	...
	public List<L2Character> getKnownCharactersByIdInRadius(int id, int radius)
	{
		List<L2Character> result = new LinkedList<>();
		for (L2Object obj : getKnownObjects().values())
		{
	...
	public List<L2Character> getKnownCharactersInRadius(L2Character origin, long radius)
	{
		List<L2Character> result = new LinkedList<>();
		for (L2Object obj : getKnownObjects().values())
		{
			if (obj instanceof L2Character)
			{
				if (Util.checkIfInRange((int) radius, origin, obj, true))
				{
					result.add((L2Character) obj);
				}
			}
