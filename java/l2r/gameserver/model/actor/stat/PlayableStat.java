...
import l2r.gameserver.model.actor.instance.L2RebirthInstance;
...
public class PlayableStat extends CharStat {
    ...
    public boolean removeExp(long value) {
        ...
        Comparator<Integer> levelComparator = Comparator.comparingInt(o -> o);

        if (getActiveChar() instanceof L2PcInstance) {
            L2PcInstance player = (L2PcInstance) getActiveChar();
            int levelToBe = getLevel() + (level - getLevel());
            int requiredCount = 0;

            boolean levelToBeInList = RebirthEngineConfigs.REBIRTH_REQUIREMENTS.containsKey(levelToBe);
            int highestLevelInList = Collections.max(RebirthEngineConfigs.REBIRTH_REQUIREMENTS.keySet(), levelComparator);

            if (levelToBeInList) {
                requiredCount = RebirthEngineConfigs.REBIRTH_REQUIREMENTS.get(levelToBe);
            } else if (levelToBe > highestLevelInList) {
                requiredCount = RebirthEngineConfigs.REBIRTH_REQUIREMENTS.get(highestLevelInList);
            }

            if (player != null && L2RebirthInstance.getRebirthLevel(player) < requiredCount) {
                String message = String.format("Você não pode ir para o nivel %d se n tiver no minimo %d rebirth.", levelToBe, requiredCount);
                CreatureSay cs = new CreatureSay(0, Say2.PARTYROOM_COMMANDER, "Level Up", message);
                player.sendMessage(message);
                player.sendPacket(cs);
                long totalExp = getExpForLevel(levelToBe) - 1 - oldExp;
                setExp(oldExp + totalExp);

                long expMaxNextLevel = getExpForLevel(getLevel() + 1);
                if ((getExp() >= expMaxNextLevel)) {
                    setExp(expMaxNextLevel - 1);
                }
                canAdd = false;
            }
        }

        if (canAdd && ((level != getLevel()) && (level >= minimumLevel))) {
            addLevel((byte) (level - getLevel()), value);
        }
        return true;
    }
