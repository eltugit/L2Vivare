...
public class PcStatus extends PlayableStatus
{
	...
	public final void reduceHp(double value, L2Character attacker, boolean awake, boolean isDOT, boolean isHPConsumption, boolean ignoreCP)
	{
		...
		if (Config.OFFLINE_MODE_NO_DAMAGE && (getActiveChar().getClient() != null) && getActiveChar().getClient().isDetached() && ((Config.OFFLINE_TRADE_ENABLE && ((getActiveChar().getPrivateStoreType() == PrivateStoreType.SELL) || (getActiveChar().getPrivateStoreType() == PrivateStoreType.PACKAGE_SELL) || (getActiveChar().getPrivateStoreType() == PrivateStoreType.BUY))) || (Config.OFFLINE_CRAFT_ENABLE && (getActiveChar().isInCraftMode() || (getActiveChar().getPrivateStoreType() == PrivateStoreType.MANUFACTURE)))))
		{
			return;
		}
		...
		if ((attacker != null) && (attacker != getActiveChar()))
		{
			...
			if (attackerPlayer != null)
			{
				...
				if (getActiveChar().isInDuel())
				{
					...
					if (attackerPlayer.getDuelId() != getActiveChar().getDuelId())
					{
						...
						// Interrupted state for attacker only if is in duel state
			...
			if ((fullValue > 0) && !isDOT)
			{
                ...
                if ((GabUtils.isInPvPInstance(getActiveChar()) && GabUtils.isInPvPInstance(attacker)) && PvPZoneManager.isAnnonym()) {
                    smsg.addString(ConfigPvPInstance.ANONYM_STRING);
                    smsg.addCharName(ConfigPvPInstance.ANONYM_STRING);
                }else if ((GabConfig.OLY_VISUAL_RESTRICTION) && attacker instanceof L2PcInstance && getActiveChar().isInOlympiadMode() && ((L2PcInstance) attacker).isInOlympiadMode()) {
                    smsg.addString(getActiveChar().getClassId().name().toUpperCase());
                    smsg.addCharName(((L2PcInstance) attacker).getClassId().name().toUpperCase());
                }else {
