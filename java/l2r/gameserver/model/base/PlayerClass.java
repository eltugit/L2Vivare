...
import static l2r.gameserver.model.base.ClassLevel.*;
import static l2r.gameserver.model.base.ClassType.*;
...
public enum PlayerClass
{
	...
	private static Set<PlayerClass> neverSubclassed;
	...
	static
	{
		...
		if(GabConfig.ALLOW_DOMINATOR_SUBS){
            neverSubclassed = EnumSet.of(Warsmith);
        }else{
	...
	public final Set<PlayerClass> getAvailableSubclasses(L2PcInstance player)
	{
		...
		if (_level == Third)
		{
			if (player.getRace() != Race.KAMAEL)
			{
				...
				{
					case ELF:
                        if(!GabConfig.ALLOW_DELF_ELF_SUBS)
				...
				{
                    if (unavailableClasses.containsAll(subclasseSet2)){
                        if (!GabConfig.ALLOW_DAG_DAG_SUBS) {
                            subclasses.removeAll(unavailableClasses);
                        }
                    }
                    if (unavailableClasses.containsAll(subclasseSet1)){
                        if (!GabConfig.ALLOW_TANK_TANK_SUBS) {
                            subclasses.removeAll(unavailableClasses);
                        }
                    }
                    if (unavailableClasses.containsAll(subclasseSet3)){
                        if (!GabConfig.ALLOW_ARC_ARC_SUBS) {
                            subclasses.removeAll(unavailableClasses);
                        }
                    }
                    if (unavailableClasses.containsAll(subclasseSet4)){
                        if (!GabConfig.ALLOW_SUM_SUM_SUBS) {
                            subclasses.removeAll(unavailableClasses);
                        }
                    }
                    if (unavailableClasses.containsAll(subclasseSet5)){
                        if (!GabConfig.ALLOW_MAG_MAG_SUBS) {
                            subclasses.removeAll(unavailableClasses);
                        }
                    }
				}
