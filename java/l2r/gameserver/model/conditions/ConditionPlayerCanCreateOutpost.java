	...
	public boolean testImpl(Env env) {
		...
		if (player.isInsideZone(ZoneIdType.SIEGE)) {
			...
			for (L2ZoneType zone : ZoneManager.getInstance().getZones(player)) {
				if (zone instanceof L2SiegeZone && castle != null) {
					L2SiegeZone siegeZone = (L2SiegeZone) zone;
					if (player.getClan().getCastleId() != 0 && siegeZone.getSettings().getSiegeableId() == player.getClan().getCastleId()) {
						player.sendMessage("You cannot summon an outpost on your own siege field.");
						canCreateOutpost = false;
						found = true;
					}
				}
