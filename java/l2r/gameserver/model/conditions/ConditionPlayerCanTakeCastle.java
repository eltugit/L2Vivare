...
public class ConditionPlayerCanTakeCastle extends Condition
{
	...
	public boolean testImpl(Env env)
	{
		...
		//check back artifact
		if (!GeoEngine.canMoveToCoord(env.getPlayer().getX(), env.getPlayer().getY() , env.getPlayer().getZ(),
				env.getTarget().getX(), env.getTarget().getY(), env.getTarget().getZ(), env.getPlayer().getInstanceId())) {
			double dist = env.getPlayer().getDistance(env.getTarget().getLocation());
			if(dist > 150) {
				env.getPlayer().sendPacket(SystemMessageId.DIST_TOO_FAR_CASTING_STOPPED);
				canTakeCastle = false;
			}
