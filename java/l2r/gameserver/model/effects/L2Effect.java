...
import scripts.handlers.effecthandlers.DispelByCategory;
import scripts.handlers.effecthandlers.DispelBySlot;
import scripts.handlers.effecthandlers.DispelBySlotProbability;
import scripts.handlers.effecthandlers.StealAbnormal;
...
public abstract class L2Effect implements IChanceSkillTrigger
{
    ...
    {
        _periodFirstTime = Math.min(newFirstTime, getAbnormalTime());
    ...
    /**
     * @return Time pass from effect start
     */
    public int getTime()
    {
    ...
    public int getRemainingTime()
    {
        ...
        int remainingTime = (getAbnormalTime() * count) - ((GameTimeController.getInstance().getGameTicks() - _periodStartTicks) / GameTimeController.TICKS_PER_SECOND);
    ...
    private final void startEffectTask()
    {
        ...
        {
            int initialDelay = Math.max((getAbnormalTime() - _periodFirstTime) * 1000, 5);
            if (_count > 1)
            {
                _currentFuture = ThreadPoolManager.getInstance().scheduleEffectAtFixedRate(new EffectTask(), initialDelay, getAbnormalTime() * 1000);
            }
            else
            {
                if(this.getClass() == DispelByCategory.class || this.getClass() == DispelBySlot.class || this.getClass() == DispelBySlotProbability.class || this.getClass() == StealAbnormal.class){
    ...
    public final void scheduleEffect(boolean force)
    {
        switch (_state)
        {
            ...
            case FINISHING:
            {
                ...
                if (getInUse() || !((_count > 1) || (getAbnormalTime() > 0)))
                {
    ...
    public boolean canBeStolen()
    {
        ...
        if(Arrays.stream(GabConfig.BUFF_ID_IGNORE_LIST).anyMatch(i -> i == this.getSkill().getId())
                && (_effector instanceof L2PcInstance)
                && (_effected instanceof L2PcInstance)
                && !(((L2PcInstance) _effector).isInOlympiadMode() && ((L2PcInstance) _effected).isInOlympiadMode())){
            return false;
        }
    ...
    {
        return "L2Effect [_skill=" + _skill + ", _state=" + _state + ", _period=" + getAbnormalTime() + "]";
    }
