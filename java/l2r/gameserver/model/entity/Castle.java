...
public final class Castle extends AbstractResidence
{
    ...
    public boolean addToTreasuryNoTax(long amount)
    {
        ...
        long newamount = Math.min(Inventory.MAX_ADENA, (_treasury + amount));
        if (updateTreasury(newamount))
        {
            _treasury = newamount;
            return true;
        }
    ...
    public boolean removeFromTreasury(long amount)
    {
        if (getOwnerId() <= 0)
        {
            return false;
        ...
        long newamount = Math.max(0, (_treasury - amount));
        if (updateTreasury(newamount))
        {
            _treasury = newamount;
            return true;
    ...
    private boolean updateTreasury(long amount)
    {
        try (Connection con = L2DatabaseFactory.getInstance().getConnection();
    ...
    {
        if (_siegeTimeRegistrationEndDate == null && castleId != 10)
        {
            _siegeTimeRegistrationEndDate = Calendar.getInstance();
        }
