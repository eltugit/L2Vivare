...
import gabriel.pvpInstanceZone.utils.PVPInstanceShowBoard;
import l2r.Config;
...
public final class Instance {
    ...
    private final Map<Integer, L2PcInstance> _playerInstances = new ConcurrentHashMap<>();
    ...
    public void setTimerIncrase(boolean value)
    {
        _isTimerIncrease = value;
    }
    ...
    public void addPlayer(int objectId) {
        ...
        _playerInstances.put(objectId, L2World.getInstance().getPlayer(objectId));
    ...
    public void removePlayer(Integer objectId) {
        ...
        _playerInstances.remove(objectId);
    ...
    public void removeItem(L2ItemInstance item) {
        _items.remove(item);
    }
    ...
    public void removePlayers() {
        if (getId() == ConfigPvPInstance.PVP_INSTANCE_INSTANCE_ID)
            PVPInstanceRewarder.getInstance().handlePvPInstanceRemovePlayers(_players, this);
        else {
    ...
    public List<L2PcInstance> getAllPlayers() {
        return new ArrayList<>(_playerInstances.values());
    }
    ...
    public void removeItems() {
        for (L2ItemInstance item : _items) {
            if (item != null) {
                L2WorldRegion region = item.getWorldRegion();
                item.decayMe();

                if (region != null) {
                    region.removeVisibleObject(item);
                }

                item.getKnownList().removeAllKnownObjects();
    ...
    protected void doCheckTimeUp(int remaining) {
        ...
        if (_players.isEmpty() && (_emptyDestroyTime == 0)) {
            ...
            interval = 300000;
            if (!_disableMessages && this.getId() != ConfigPvPInstance.PVP_INSTANCE_INSTANCE_ID) {
    ...
    private Location _spawnlocpvp = null;

    public Location getSpawnLocPvP() {
        return _spawnlocpvp;
    }

    public void setSpawnLocPvP(Location loc) {
        _spawnlocpvp = loc;
    }
