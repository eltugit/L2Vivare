...
public abstract class AbstractOlympiadGame
{
	...
	protected L2OlympiadStadiumZone _zone;
	...
	protected final void addPointsToParticipant(Participant par, int points)
	{
        ...
        if (GabConfig.OLY_VISUAL_RESTRICTION) {
            sm.addString(par.getPlayer().getClassId().name().toUpperCase());
        } else {
	...
	protected static final void removals(L2PcInstance player, boolean removeParty)
	{
		try
		{
            ...
            if(GabConfig.CLASS_ID_NERF_HEAVY.stream().anyMatch(e-> e == player.getClassId().getId())){
                ...
                if(skill == null){
                    ...
                    if(player.getKnownSkill(GabConfig.CLASS_HEAVY_NERF_SKILL) != null)
                        player.removeSkill(skill, false);
                }
            }
            if(GabConfig.CLASS_ID_NERF_BOW_USAGE.stream().anyMatch(e-> e == player.getClassId().getId())){
                L2Skill skill = SkillData.getInstance().getSkill(GabConfig.BOW_USAGE_NERF_SKILL, 1);
                if(skill == null){
                    System.out.println("Skill with ID: "+ GabConfig.BOW_USAGE_NERF_SKILL + " dont exist!");
                }else{
                    if(player.getKnownSkill(GabConfig.BOW_USAGE_NERF_SKILL) != null)
                        player.removeSkill(skill, false);
                }
            }
	...
	protected static final void restorePlayer(L2PcInstance player)
	{
		if (player.isDead())
		{
	...
	public void setZone(L2OlympiadStadiumZone zone)
	{
		_zone = zone;
	}
	...
	public abstract void broadcastPacket(L2GameServerPacket packet);

	public abstract boolean needBuffers();

	public abstract boolean checkDefaulted();

	public abstract void removals();

	public abstract boolean portPlayersToArena(List<Location> spawns);

	public abstract void cleanEffects();

	public abstract void restorePlayers();
	...
	public abstract boolean checkBattleStatus();

	public abstract boolean haveWinner();

	public abstract void validateWinner(L2OlympiadStadiumZone stadium);
    ...
    public abstract void protectAgainstInvites(boolean bool);
