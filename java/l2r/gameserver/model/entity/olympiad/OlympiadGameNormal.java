...
public abstract class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	public final void removals()
	{
        ...
        if (GabConfig.OLY_VISUAL_RESTRICTION) {
            _playerOne.getPlayer().sendPacket(new CreatureSay(0, Say2.TELL, _playerOne.getPlayer().getName(), "Your opponent class is " + _playerTwo.getPlayer().getClassId().name().toUpperCase()));
            _playerTwo.getPlayer().sendPacket(new CreatureSay(0, Say2.TELL, _playerTwo.getPlayer().getName(), "Your opponent class is " + _playerOne.getPlayer().getClassId().name().toUpperCase()));
        }
	...
	public void validateWinner(L2OlympiadStadiumZone stadium)
	{
		...
		if (_pOneCrash || _pTwoCrash)
		{
			try
			{
				if (_pTwoCrash && !_pOneCrash)
				{
                    ...
                    if (GabConfig.OLY_VISUAL_RESTRICTION) {
                        sm.addString(_playerOne.getPlayer().getClassId().name().toUpperCase());
                    } else {
				...
				else if (_pOneCrash && !_pTwoCrash)
				{
					sm = SystemMessage.getSystemMessage(SystemMessageId.C1_HAS_WON_THE_GAME);
                    if (GabConfig.OLY_VISUAL_RESTRICTION) {
                        sm.addString(_playerTwo.getPlayer().getClassId().name().toUpperCase());
                    } else {
		...
		try
		{
			...
			else if ((_playerTwo.getPlayer() == null) || !_playerTwo.getPlayer().isOnline() || ((playerTwoHp == 0) && (playerOneHp != 0)) || ((_damageP1 > _damageP2) && (playerTwoHp != 0) && (playerOneHp != 0)))
			{
				...
				list2.add(new OlympiadInfo(_playerTwo.getName(), _playerTwo.getClanName(), _playerTwo.getClanId(), _playerTwo.getBaseClass(), _damageP2, playerTwoPoints - pointDiff, -pointDiff));
                if (GabConfig.OLY_VISUAL_RESTRICTION) {
                    winner = _playerOne.getPlayer().getClassId().name().toUpperCase() + " won";
                } else {
			...
			else if ((_playerOne.getPlayer() == null) || !_playerOne.getPlayer().isOnline() || ((playerOneHp == 0) && (playerTwoHp != 0)) || ((_damageP2 > _damageP1) && (playerOneHp != 0) && (playerTwoHp != 0)))
			{
                ...
                if (GabConfig.OLY_VISUAL_RESTRICTION) {
                    winner = _playerTwo.getPlayer().getClassId().name().toUpperCase() + " won";
                } else {
	...
	{
        if (GabConfig.OLY_VISUAL_RESTRICTION) {
            return new String[]{_playerOne.getPlayer().getClassId().name().toUpperCase(), _playerTwo.getPlayer().getClassId().name().toUpperCase()};
        } else {
    ...
    public final void protectAgainstInvites(boolean bool) {
        if (_playerOne.getPlayer() != null) _playerOne.getPlayer().set_waitTimeTeleporterOly(bool);
        if (_playerTwo.getPlayer() != null) _playerTwo.getPlayer().set_waitTimeTeleporterOly(bool);
    }
}
