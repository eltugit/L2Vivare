...
public class OlympiadGameTeams extends AbstractOlympiadGame
{
	...
	public final boolean portPlayersToArena(List<Location> spawns)
	{
		...
		try
		{
			for (int i = 0; i < _teamOneSize; i++)
			{
				setOlympiadGame(_teamOne[i], this);
			...
			for (int i = 0; i < _teamTwoSize; i++)
			{
				setOlympiadGame(_teamTwo[i], this);
	...
	{
		for (Participant plr : _teamOne)
		{
			final L2PcInstance player = plr.getPlayer();
			player.setCurrentCp(player.getMaxCp());
			player.setCurrentHp(player.getMaxHp());
			player.setCurrentMp(player.getMaxMp());
		}
		
		for (Participant plr : _teamTwo)
		{
			final L2PcInstance player = plr.getPlayer();
			player.setCurrentCp(player.getMaxCp());
			player.setCurrentHp(player.getMaxHp());
    ...
    public final void protectAgainstInvites(boolean bool) {
        for (Participant player : _teamOne) {
            if (player.getPlayer() != null) player.getPlayer().set_waitTimeTeleporterOly(bool);
        }
        for (Participant player : _teamTwo) {
            if (player.getPlayer() != null) player.getPlayer().set_waitTimeTeleporterOly(bool);
        }
    }
}
