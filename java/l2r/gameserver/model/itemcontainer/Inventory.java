...
public abstract class Inventory extends ItemContainer
{
	...
	private static final class ItemSkillsListener implements PaperdollListener
	{
		...
		public void notifyUnequiped(int slot, L2ItemInstance item, Inventory inventory)
		{
			...
			for (SkillHolder skillInfo : it.getSkills())
			{
	...
	public int getPaperdollItemVisualDisplayId(int slot)
	{
		...
		if (item != null)
		{
			if (item.getVisualItemId() > 0 && item.getVisualItemId() != 59999)
			{
				return item.getVisualItemId();
			}else if (item.getVisualItemId() == -1 || item.getVisualItemId() == 59999){
			    return 0;
            }
	...
	public void equipItem(L2ItemInstance item)
	{
		...
		switch (targetSlot)
		{
			...
			case L2Item.SLOT_R_HAND:
			{
                ...
                L2ItemInstance lh = getPaperdollItem(PAPERDOLL_LHAND);
                if(lh != null){
                    if(lh.getItemType() == EtcItemType.ARROW && item.getItemType() != WeaponType.BOW){
                        setPaperdollItem(PAPERDOLL_LHAND, null);
                    }else if(lh.getItemType() == EtcItemType.BOLT && item.getItemType() != WeaponType.CROSSBOW){
                        setPaperdollItem(PAPERDOLL_LHAND, null);
                    }else if(lh.getItemType() == EtcItemType.LURE && item.getItemType() != WeaponType.FISHINGROD){
                        setPaperdollItem(PAPERDOLL_LHAND, null);
                    }
                }
                setPaperdollItem(PAPERDOLL_RHAND, item);
