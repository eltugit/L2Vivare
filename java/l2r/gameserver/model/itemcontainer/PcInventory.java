...
public class PcInventory extends Inventory
{
    ...
    public static int[][] restoreVisibleInventory(int objectId)
    {
        ...
        try (Connection con = L2DatabaseFactory.getInstance().getConnection();
             PreparedStatement statement2 = con.prepareStatement("SELECT object_id,item_id,loc_data,enchant_level,visual_item_id FROM items WHERE owner_id=? AND loc='PAPERDOLL' ORDER BY 3"))
        {
            statement2.setInt(1, objectId);
            boolean dressFound = false;
            ...
            try (ResultSet invdata = statement2.executeQuery())
            {
                while (invdata.next())
                {
                    ...
                    if(dressFound && (slot == 6 || slot == 10 || slot == 11 || slot == 12))
                        continue;

                    int itemId = invdata.getInt("item_id");
                    int visual = invdata.getInt("visual_item_id");

                    if(!dressFound && ArmorSetVisualiser.isChest(itemId) && visual > 0 && slot == 6){
                        dressFound = true;
                        chestVisual = visual;
                        //6 == armor
                        paperdoll[PAPERDOLL_CHEST][1] = ArmorSetVisualiser.getIntForPcInventory(itemId, chestVisual);
                        //gloves
                        paperdoll[Inventory.PAPERDOLL_GLOVES][1] = ArmorSetVisualiser.getIntForPcInventorySLOT(Inventory.PAPERDOLL_GLOVES, chestVisual);
                        //boots
                        paperdoll[Inventory.PAPERDOLL_FEET][1] = ArmorSetVisualiser.getIntForPcInventorySLOT(Inventory.PAPERDOLL_FEET, chestVisual);
                        //legs
                        paperdoll[Inventory.PAPERDOLL_LEGS][1] = ArmorSetVisualiser.getIntForPcInventorySLOT(Inventory.PAPERDOLL_LEGS, chestVisual);
                    }else {
                        if(!dressFound && (slot == 6 || slot == 10 || slot == 11 || slot == 12) && ArmorSetVisualiser.isOtherPieces(itemId)){
                            paperdoll[slot][1] = itemId;
                        }
                        else{
                            paperdoll[slot][1] = visual > 0 ? visual : itemId;
                        }
                    }
