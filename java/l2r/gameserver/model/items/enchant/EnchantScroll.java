...
public final class EnchantScroll extends AbstractEnchantItem
{
	...
	private int[] specialScrolls = { // ELTON add enchants speciais q vao ter o enchant variavel aki.
			22221,
			22222,
			22223,
			22224,
	...
	public EnchantScroll(StatsSet set)
	{
		...
		isSpecial = Arrays.stream(specialScrolls).anyMatch(e->e==getItem().getId());
	}
	...
	public double getChance(L2PcInstance player, L2ItemInstance enchantItem)
	{
		...
		if(player.isPremium()){
		    chance = chance + GabConfig.PREMIUM_PLAYER_EXTRA_ENCHANT_CHANCE;
        }
		return chance;
	}
	...
	{
		if(isSpecial()) // special sempre vai succedir
