...
public class Quest extends AbstractScript implements IIdentifiable
{
	...
	public static final String TUTORIAL = "Q00255_Tutorial";
	...
	public Quest(int questId, String desc)
	{
		_questId = questId;
		_name = getClass().getSimpleName();
		_descr = desc;
	...
	{
		final String result = HtmCache.getInstance().getHtm(player, player.getHtmlPrefix(), "data/html/noquest.htm");
	...
	{
		final String result = HtmCache.getInstance().getHtm(player, player.getHtmlPrefix(), "data/html/alreadycompleted.htm");
		if ((result != null) && (result.length() > 0))
		{
			return result;
		}
	...
	public String showHtmlFile(L2PcInstance player, String filename, L2Npc npc)
	{
		...
		String content = getHtm(player, player.getHtmlPrefix(), filename);
	 ...
	 */
	public String getHtm(L2PcInstance activeChar, String prefix, String fileName)
	{
		final HtmCache hc = HtmCache.getInstance();
		String content = hc.getHtm(activeChar, prefix, fileName.startsWith("data/") ? fileName : "data/scripts/" + getDescr().toLowerCase() + "/" + getName() + "/" + fileName);
		if (content == null)
		{
			content = hc.getHtm(activeChar, prefix, "data/scripts/" + getDescr() + "/" + getName() + "/" + fileName);
			if (content == null)
			{
				content = hc.getHtmForce(activeChar, prefix, "data/scripts/quests/" + getName() + "/" + fileName);
			}
		}
		return content;
	}
    ...
    public boolean checkIPs(L2Party party) {
        if (party != null) {
            for (L2PcInstance player : party.getMembers()) {
                final String playerIP = player.getIPAddress();

                if (playerIP != null) {
                    for (L2PcInstance player2 : party.getMembers()) {
                        if (player == player2)
                            continue;

                        if (player2.getIPAddress() == null || playerIP.equalsIgnoreCase(player2.getIPAddress())) {
                            player2.sendMessage(player2.getName() + " has the same IP as " + player + " and thus cannot enter the instance");
                            return false;
                        }
                    }
                }
