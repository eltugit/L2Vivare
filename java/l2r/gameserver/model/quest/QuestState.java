...
public final class QuestState
{
	...
	public void SetFlagJournal(int value)
	{
		int completedStateFlags = 0;
		completedStateFlags |= (1 << 31);
		completedStateFlags |= (1 << (value - 1));
		set("__compltdStateFlags", String.valueOf(completedStateFlags));
		
		_player.sendPacket(new QuestList());
		
		_player.sendPacket(new ExShowQuestMark(getQuest().getId()));
	}
	
	public void ShowQuestMark(L2Character creature, int questId)
	{
		if (creature == null)
		{
			return;
		...
		creature.getActingPlayer().sendPacket(new ExShowQuestMark(questId));
	}
	...
	public void showTutorialHTML(String html)
	{
		try
		{
			String text = HtmCache.getInstance().getHtm(_player, _player.getHtmlPrefix(), "data/scripts/quests/" + Quest.TUTORIAL + "/" + html);
			if (text == null)
			{
				_log.warn("missing html page data/scripts/quests/" + Quest.TUTORIAL + "/" + html);
				text = "<html><body>File data/scripts/quests/" + Quest.TUTORIAL + "/" + html + " not found or file is empty.</body></html>";
			}
