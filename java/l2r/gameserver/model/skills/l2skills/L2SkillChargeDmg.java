	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		for (L2Character target : (L2Character[]) targets)
		{
			...
			if (defence != -1)
			{
				...
				double damageMultiplier = Formulas.calcValakasTrait(attacker, target, skill) * Formulas.calcWeaponTraitBonus(attacker, target) * Formulas.calcAttributeBonus(attacker, target, skill);
			...
			damage = BalanceGabrielHandler.getInstance().calcMagicDam(attacker, target, damage, critical, skill.isMagic());
