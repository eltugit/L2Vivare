...
public class L2SkillDrain extends L2Skill {
	...
	public void useSkill(L2Character activeChar, L2Object[] targets) {
		...
		for (L2Character target : (L2Character[]) targets) {
			if (target.isAlikeDead() && (getTargetType() != L2TargetType.CORPSE_MOB && getTargetType() != L2TargetType.CORPSE_PET_MOB)) {
				continue;
			}
			...
			// Check for hp restore limitations cause of debuffs
			hp = Math.max(Math.min(hp, activeChar.getMaxRecoverableHp()), activeChar.getCurrentHp());
			if (hp != 0) {
				activeChar.setCurrentHp(hp);
			}

			// Check to see if we should damage the target
			if ((damage > 0) && (!target.isDead() || (getTargetType() != L2TargetType.CORPSE_MOB && getTargetType() != L2TargetType.CORPSE_PET_MOB))) {
				// Manage attack or cast break of the target (calculating rate, sending message...)
				if (!target.isRaid() && Formulas.calcAtkBreak(target, damage, this)) {
					target.breakAttack();
					target.breakCast();
				}
				...
				if (hasEffects() && (getTargetType() != L2TargetType.CORPSE_MOB && getTargetType() != L2TargetType.CORPSE_PET_MOB)) {
					...
					if ((Formulas.calcSkillReflect(target, this) & Formulas.SKILL_REFLECT_SUCCEED) > 0) {
						...
						if (Formulas.calcSkillSuccess(activeChar, target, this, shld, ss, sps, bss)) {
							...
							if ((GabUtils.isInPvPInstance(activeChar) && GabUtils.isInPvPInstance(target)) && PvPZoneManager.getInstance().isAnnonym()) {
								sm.addCharName(ConfigPvPInstance.ANONYM_STRING);
							} else if ((GabConfig.OLY_VISUAL_RESTRICTION) && activeChar instanceof L2PcInstance && target instanceof L2PcInstance && ((L2PcInstance) activeChar).isInOlympiadMode() && ((L2PcInstance) target).isInOlympiadMode()) {
			...
			if (target.isDead() && ((getTargetType() == L2TargetType.CORPSE_MOB && target.isNpc()) || (getTargetType() == L2TargetType.CORPSE_PET_MOB) && (target.isNpc() || target.isSummon() || target.isServitor()))) {
				if(target.isNpc()) {
					((L2Npc) target).endDecayTask();
				}else if(target.isSummon() || target.isServitor()){
