...
public class L2GameClient extends MMOClient<MMOConnection<L2GameClient>>
{
			...
			try
			{
				if ((getActiveChar() != null) && !isDetached())
				{
					...
					if (offlineMode(getActiveChar()))
					{
                        ...
                        if(getActiveChar().isInTournament()) {
                            TTournamentManager.getInstance().onDisconnect(getActiveChar());
                        }
		...
		public void run()
		{
			try
			{
                ...
                if (getActiveChar() != null) // this should only happen on connection loss
				{
                    ...
                    PvPZoneManager.handleLeavePvPZone(player, true);
    ...
    public final String getNetIp(){
	    return isDetached() ? getConnection().getInetAddress().getHostAddress() : null;
    }
