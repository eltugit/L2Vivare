...
import scripts.handlers.itemhandlers.DressMeItem;
...
public final class DlgAnswer extends L2GameClientPacket
{
	...
	//TODO GABRIEL Handles DLG Answer button
	@Override
	public void runImpl()
	{
		...
		}else if (_messageId == SystemMessageId.S1.getId() && activeChar.getItemDlg() != null)
        {
            if(_answer == 1){
                DressMeItem.useItem(activeChar,activeChar.getItemDlg(), true);
            }else{
                activeChar.setItemDlg(null);
            }
        }else if (_messageId == SystemMessageId.S1.getId() && activeChar.isAskedToJoinEvent())
        {
            if(_answer == 1){
                EventManager.getInstance().getMainEventManager().registerPlayer(activeChar.getEventInfo());
            }else{
                activeChar.setAskedToJoinEvent(false);
            }
        }else if (_messageId == SystemMessageId.S1.getId() && activeChar.hasDialogAskActive())
        {
