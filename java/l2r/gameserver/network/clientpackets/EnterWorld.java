...
public class EnterWorld extends L2GameClientPacket
{
	...
	// TODO GABRIEL check this for internal netwerk IP of player!
	...
	protected void runImpl()
	{
		...
		IPLog.auditGMAction(activeChar.getName(), activeChar.getClient().getConnection().getInetAddress().getHostAddress(), activeChar.getClient().getHWID());
		HWID.auditGMAction(activeChar.getClient().getHWID(), activeChar.getName());
		...
		if (GabConfig.SHOW_WELCOME_PM)
		{
			for (String s : GabConfig.PM_TEXT)
			{
				CreatureSay np = new CreatureSay(0, Say2.TELL, GabConfig.PM_FROM, s);
				activeChar.sendPacket(np);
			}
			
		}
		if (GabConfig.SHOW_WELCOME_PMENG)
		{
			for (String s : GabConfig.PM_TEXTENG)
			{
				CreatureSay np = new CreatureSay(0, Say2.TELL, GabConfig.PM_FROMENG, s);
				activeChar.sendPacket(np);
			}
		}
		...
		CharCustomHeroTable.getInstance().process(activeChar);
		if (activeChar.getClan() != null)
		{
			if (activeChar.getClan().isVip())
			{
				onEnterClanVip(activeChar, activeChar.getClan());
			}
		}
		...
		if (GabConfig.CSKOTH_EVENT_ENABLED) CSKOTHEvent.onLogin(activeChar);
		...
		if (GabConfig.CLASS_ID_NERF_HEAVY.stream().anyMatch(e -> e == activeChar.getClassId().getId()))
		{
		...
		if (GabConfig.CLASS_ID_NERF_BOW_USAGE.stream().anyMatch(e -> e == activeChar.getClassId().getId()))
		{
...
//		MuseumManager.getInstance().giveReward(activeChar);
	...
	private void onEnterClanVip(L2PcInstance activeChar, L2Clan clan)
	{
		long now = Calendar.getInstance().getTimeInMillis();
		long endDay = clan.getVipEndTime();
		if (now > endDay)
		{
			clan.setVip(false);
			clan.setVipEndTime(0);
			clan.updateClanVIPInDB();
			activeChar.sendMessage("Removed CLAN Vip stats... period ends ");
		}
		else
		{
			...
			if (_daysleft > 30)
			{
				activeChar.sendMessage("Vip CLAN period ends in " + df.format(dt) + ". enjoy the Game");
			}
			else if (_daysleft > 0)
			{
				activeChar.sendMessage("left " + (int) _daysleft + " days for CLAN VIP period ends");
			}
			else if (_daysleft < 1)
			{
				long hour = (endDay - now) / 3600000;
				activeChar.sendMessage("left " + (int) hour + " hours to CLAN VIP period ends");
			}
		}
	}
