...
public final class RequestAnswerJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (GabUtils.isInPvPInstance(requestor) && !GabUtils.isInPvPInstance(player)) {
			return;
		}
		if (!GabUtils.isInPvPInstance(requestor) && GabUtils.isInPvPInstance(player)) {
			return;
		...
		if (GabUtils.isInPvPInstance(requestor) && GabUtils.isInPvPInstance(player)) {
			if (requestor.getTeam() != player.getTeam()) {
				return;
			}
			if (player.getTeam4t() != requestor.getTeam4t()) {
				return;
			}
		...
		if (GabUtils.isInPvPInstance(requestor) && PvPZoneManager.getInstance().isAnnonym()) {
			return;
		}

		if(!GabUtils.canJoinParty(requestor, player))
			return;
		...
		switch (_response)
		{
			...
			{
				if(player.isInTournament() || requestor.isInTournament()) {
					return;
				}
