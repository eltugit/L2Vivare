...
public final class RequestBypassToServer extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			if (_command.startsWith(possibleNonHtmlCommand) || startsWith(_command, CommunityClan.getInstance().getBypassCommands()))
			{
		...
		try
		{
			...
			else if(startsWith(_command, CommunityClan.getInstance().getBypassCommands())){
                CommunityClan.getInstance().onBypassCommand(activeChar, _command);
            }
			...
			else if (_command.startsWith("_certi"))
			{
				CertificationManager.getInstance().parseCommand(_command, activeChar);
				return;
			}else if (_command.startsWith("gab_multisell"))
			{
			    String[] command = _command.split(" ");

			    if(command.length < 2){
                    System.out.println("Command gab_multisell is missing a parameter: Usage: bypass -h gab_multisell multisellId");
                    return;
                }
                int multi = Integer.parseInt(command[1]);
                if (CommunityServicesConfigs.MULTISELL_LIST.contains(multi)) {
                    ...
                    System.out.println(activeChar.getName()+" tried to access a multisell: "+multi +"that is not in the CB multisell list.");
                }
