...
public final class RequestEnchantItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		synchronized (item)
		{
			...
			TempEnchant tempEnchant = GabConfig.TEMP_ENCHANT.get(scroll.getId());
			boolean tempScroll = tempEnchant != null;
			...
			if (!tempScroll) {
				switch (resultType)
				{
					...
					case SUCCESS:
					{
						...
						int amountToEnchant = item.getEnchantLevel() + 1;
						final int previousEnchant = item.getEnchantLevel();
						boolean special = scrollTemplate.isSpecial();
						if(special){
							int chance = Rnd.get(100);
							if(chance >= 85) //  15% +22
								amountToEnchant = 22;
							else if(chance >= 50) // 35% +21
								amountToEnchant = 21;
							else 				// 50% +20
								amountToEnchant = 20;
						}
						...
						if (special || scrollTemplate.getChance(activeChar, item) > 0)
						{
							item.setEnchantLevel(special ? amountToEnchant : item.getEnchantLevel() + 1);
							item.updateDatabase();
						}
						if(special && item.getEnchantLevel() <= previousEnchant){
				...
				if (resultType == EnchantResultType.ERROR) {
					...
					L2ItemInstance tempItem = new L2ItemInstance(IdFactory.getInstance().getNextId(), item, true);
					tempItem.setEnchantLevel(tempEnchant.getValue());
					tempItem.setTime(System.currentTimeMillis() + (tempEnchant.getTime() * TimeUtils.DAY_IN_MILLIS));
					tempItem.scheduleLifeTimeTask();
					activeChar.addItem("Temp Item", tempItem, null, true);
					activeChar.store();
					tempItem.updateDatabase();
				}
			}
