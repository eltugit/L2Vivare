...
public final class RequestExAskJoinMPCC extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		int countMe = getPlayers(activeChar);
		int countThem = getPlayers(player);

		if(countMe + countThem >= GabConfig.MAX_PT_CC){
			activeChar.sendMessage("You cannot invite this party to the CC because the maximum number of players have been reached: "+GabConfig.MAX_PT_CC);
			return;
		}
	...
	private int getPlayers(L2PcInstance pl){
		if(pl.isInParty()){
			if(pl.getParty().isInCommandChannel()){
//                return pl.getParty().getCommandChannel().getMemberCount();
				return pl.getParty().getCommandChannel().getPartys().size();
			}else{
				return 1;
			}
		}else{
