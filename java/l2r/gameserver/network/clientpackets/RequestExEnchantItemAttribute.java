...
public class RequestExEnchantItemAttribute extends L2GameClientPacket
{
	...
	public int getLimit(L2ItemInstance item, int stoneId)
	{
		...
		return elementItem == null ? 0 : item.isWeapon() ? Elementals.WEAPON_VALUES[getMax(elementItem._type._maxLevel, GabConfig.ELEMENTAL_LEVEL_WEAPON)] : Elementals.ARMOR_VALUES[getMax(elementItem._type._maxLevel, GabConfig.ELEMENTAL_LEVEL_ARMOR)];
	}

    private int getMax(int elementalValue, int limitValue) {
        if (GabConfig.ELEMENTAL_CUSTOM_LEVEL_ENABLE) {
