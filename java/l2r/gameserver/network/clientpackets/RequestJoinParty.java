...
public final class RequestJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
        ...
        if (requestor.isInTournament()) {
            requestor.sendMessage("You can't invite a player while Registered on event");
            return;
        }
        if (target.isInTournament()) {
            requestor.sendMessage("You can't invite a player that is Registered on event");
            return;
        }

        if (GabUtils.isInPvPInstance(requestor) && !GabUtils.isInPvPInstance(target)) {
            requestor.sendMessage("You can't invite a player that is outside the pvp instance zone if you are inside it.");
            return;
        }
        if (!GabUtils.isInPvPInstance(requestor) && GabUtils.isInPvPInstance(target)) {
            requestor.sendMessage("Your target is inside the PVP Zone. You cannot invite him. Enter the zone so you can invite him.");
            return;
        }
        if(GabUtils.isInPvPInstance(requestor) && GabUtils.isInPvPInstance(target)){
            if (requestor.getTeam() != target.getTeam()) {
                requestor.sendMessage("You can't invite a player that is in the Enemy team!");
                return;
            }
            if(target.getTeam4t() != requestor.getTeam4t()){
                requestor.sendMessage("Your target is in the Enemy team, you cannot invite him/her!");
                return;
            }
            if(PvPZoneManager.isAnnonym()){
                requestor.sendMessage("Cannot invite inside Annonym Zone!");
                return;
            }
        }
...
//		if (!party.isLeader(requestor) && !(GabUtils.isInPvPInstance(target) && GabUtils.isInPvPInstance(requestor)))
//		{
