...
public final class RequestSendPost extends L2GameClientPacket
{
    ...
    public void set_items(AttachmentItem[] _items) {
        this._items = _items;
    }

    public void set_isCod(boolean _isCod) {
        this._isCod = _isCod;
    }
	...
	public void runImpl()
	{
		...
		if(isAdminSend){
            ...
            int adminObjID = GabConfig.ER_EVENT_ADMIN_OBJ_ID;
            Message msg = new Message(adminObjID, receiverId, _isCod, _subject, _text, _reqAdena);
            L2PcInstance player = L2World.getInstance().getPlayer(adminObjID);
            if(player == null || !player.isOnline()){
                player = L2PcInstance.load(GabConfig.ER_EVENT_ADMIN_OBJ_ID);
            }
            if(removeItemsAdmin(player, msg)){
	...
	private final boolean removeItemsAdmin(L2PcInstance player, Message msg)
	{
        if (_items == null)
        {
            return true;
