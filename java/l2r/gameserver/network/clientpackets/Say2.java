...
public final class Say2 extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			if (activeChar.isChatBanned() && Util.contains(Config.BAN_CHAT_CHANNELS, 0))
			{
				activeChar.sendPacket(SystemMessageId.CHATTING_IS_CURRENTLY_PROHIBITED);
				return;
			}
			if(activeChar.usedPremiumChat > 0 && GabUtils.getSecondsFromStart(activeChar.usedPremiumChat) < 10){
				activeChar.sendMessage("Please wait 10 seconds before using this chat again");
				return;
			}

			if (_type == 0)
			{
				Broadcast.toAllOnlinePlayers(new CreatureSay(0, 20, PremiumServiceConfigs.PREMIUM_NAME_PREFIX + activeChar.getName(), _text.substring(1)));
				activeChar.usedPremiumChat = System.currentTimeMillis();
			}
			return;
		}
		...
		if (ChatBanManager.getInstance().checkBannedTime(activeChar.getAccountName(), activeChar.getClient().getHWID()))
		{
			activeChar.sendMessage("Your chat was suspended for " + ChatBanManager.banTime(ChatBanManager.getInstance().getTimeInfo(activeChar.getAccountName(), activeChar.getClient().getHWID())));
