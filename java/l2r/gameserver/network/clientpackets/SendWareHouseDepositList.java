...
public final class SendWareHouseDepositList extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if(!isInterServer){
			...
			isPrivate = warehouse instanceof PcWarehouse;

			if (((manager == null) || !manager.isWarehouse() || !manager.canInteract(player)) && !player.isGM() && !player.getQuickVarB(QuickVarType.PORTAL_WH.getCommand(), false)) {
				return;
			}
		...
		for (ItemHolder i : _items) {
			...
			if ((!oldItem.isDepositable(isPrivate) && !oldItem.isHeroItem()) || !oldItem.isAvailable(player, true, isPrivate)) {
				continue;
			}

			player.destroyItem("ISE", oldItem, i.getCount(), player, true);
			L2ItemInstance tmp = new L2ItemInstance(IdFactory.getInstance().getNextId(), oldItem, true);
			tmp.setCount(i.getCount());
			InterServerExchangeManager.getInstance().add(player, tmp);
