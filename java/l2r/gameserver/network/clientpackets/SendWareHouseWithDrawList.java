...
import l2r.gameserver.model.actor.instance.L2InterServerExchangeInstance;
...
public final class SendWareHouseWithDrawList extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        boolean isInterServer = manager instanceof L2InterServerExchangeInstance;
        ...
        if(!isInterServer){
            ...
            if (manager == null) {
                if (!player.getQuickVarB(QuickVarType.PORTAL_WH.getCommand(), false) && !player.isGM()) {
                    return;
                }
            } else {
                if (!manager.isWarehouse() && !player.getQuickVarB(QuickVarType.PORTAL_WH.getCommand(), false) && !player.isGM()) {
                    return;
                } else if (!manager.canInteract(player) && !player.isGM()) {
                    return;
                }
            }
        ...
        final InventoryUpdate iu = Config.FORCE_INVENTORY_UPDATE ? null : new InventoryUpdate();
        for (ItemHolder i : _items) {
            L2ItemInstance oldItem = (L2ItemInstance) L2World.getInstance().findObject(i.getId());
            if ((oldItem == null) || (oldItem.getCount() < i.getCount())) {
                _log.warn("Error withdrawing a warehouse object for char " + player.getName() + " (olditem == null)");
                return;
            }
            final long depositedcount = oldItem.getCount();
            final long requestedCount = i.getCount();

            final long newCount = depositedcount - requestedCount;

            oldItem.setCount(requestedCount);
            player.addItem("IES", oldItem, player, true);

            if (iu != null) {
                if (newCount > 0) {
                    oldItem.setCount(newCount);
                    InterServerExchangeManager.getInstance().update(player, oldItem.getId(), newCount, false);
                    iu.addModifiedItem(oldItem);
                } else {
                    oldItem.setCount(depositedcount);
                    InterServerExchangeManager.getInstance().delete(player, oldItem);
                    iu.addNewItem(oldItem);
                }
            }
        }
