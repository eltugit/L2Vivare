...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		try
		{
			...
			if (activeChar.isGM())
			{
				activeChar.sendMessage(item.getItem().getName() + " , ID: " + item.getItem().getId());
			}
			...
			// Flood protect UsePotion
			if (_itemId == 728)
			{
				if (!getClient().getFloodProtectors().get_usePotion728().tryPerformAction("use potion728"))
				{
					return;
				}
			}
			else if (_itemId == 5592)
			{
				if (!getClient().getFloodProtectors().get_usePotion5592().tryPerformAction("use potion5592"))
				{
					return;
				}
			}
			else if (_itemId == 5591)
			{
				if (!getClient().getFloodProtectors().get_usePotion5591().tryPerformAction("use potion5591"))
				{
					return;
				}
			}
			else if (_itemId == 10410)
			{
				if (!getClient().getFloodProtectors().get_usePotion10410().tryPerformAction("use potion10410"))
				{
					return;
				}
			}
			else if (_itemId == 1539)
			{
				if (!getClient().getFloodProtectors().get_usePotion1539().tryPerformAction("use potion1539"))
				{
					return;
				}
			}
			...
			/*
			 * Alt game - Karma punishment // SOE 736 Scroll of Escape 1538 Blessed Scroll of Escape 1829 Scroll of Escape: Clan Hall 1830 Scroll of Escape: Castle 3958 L2Day - Blessed Scroll of Escape 5858 Blessed Scroll of Escape: Clan Hall 5859 Blessed Scroll of Escape: Castle 6663 Scroll of
			 * Escape: Orc Village 6664 Scroll of Escape: Silenos Village 7117 Scroll of Escape to Talking Island 7118 Scroll of Escape to Elven Village 7119 Scroll of Escape to Dark Elf Village 7120 Scroll of Escape to Orc Village 7121 Scroll of Escape to Dwarven Village 7122 Scroll of Escape to
			 * Gludin Village 7123 Scroll of Escape to the Town of Gludio 7124 Scroll of Escape to the Town of Dion 7125 Scroll of Escape to Floran 7126 Scroll of Escape to Giran Castle Town 7127 Scroll of Escape to Hardin's Private Academy 7128 Scroll of Escape to Heine 7129 Scroll of Escape to the
			 * Town of Oren 7130 Scroll of Escape to Ivory Tower 7131 Scroll of Escape to Hunters Village 7132 Scroll of Escape to Aden Castle Town 7133 Scroll of Escape to the Town of Goddard 7134 Scroll of Escape to the Rune Township 7135 Scroll of Escape to the Town of Schuttgart. 7554 Scroll of
			 * Escape to Talking Island 7555 Scroll of Escape to Elven Village 7556 Scroll of Escape to Dark Elf Village 7557 Scroll of Escape to Orc Village 7558 Scroll of Escape to Dwarven Village 7559 Scroll of Escape to Giran Castle Town 7618 Scroll of Escape - Ketra Orc Village 7619 Scroll of
			 * Escape - Varka Silenos Village 10129 Scroll of Escape : Fortress 10130 Blessed Scroll of Escape : Fortress
			 */
			if (item.isScroll() && CSKOTHEvent.isPlayerParticipant(activeChar.getObjectId()) && !CSKOTHEvent.onScrollUse(activeChar.getObjectId())) {
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
			if ((!Config.ALT_GAME_KARMA_PLAYER_CAN_TELEPORT && (activeChar.getKarma() > 0)) || GabUtils.isInPvPInstance(activeChar) || activeChar.isInArenaEvent() || activeChar.isInTournament())
			{
				switch (_itemId)
				{
					case 736:
				...
				if ((_itemId >= 13395) && (_itemId <= 13414))
				{
					return;
				}
				if ((_itemId >= 13731) && (_itemId <= 13739))
				{
					return;
				}
				if ((_itemId >= 20372) && (_itemId <= 20390))
				{
					return;
				}
			}
			...
			if (item.isEquipable())
			{
				...
				IItemHandler handlerDrr = ItemHandler.getInstance().getHandler(item.getItem());
				
				if ((handlerDrr == null) && !activeChar.isInOlympiadMode())
				{
					if (!GabConfig.ALLOW_HEAVY_USE_LIGHT)
					{
						if (GabConfig.NOTALLOWEDUSELIGHT.contains(activeChar.getClassId().getId()))
						{
							if (item.getItemType() == ArmorType.LIGHT)
							{
								activeChar.sendMessage("You can use light only in Oly!");
								return;
							}
						}
					}
					...
					{
						if (GabConfig.NOTALLOWEDUSEHEAVY.contains(activeChar.getClassId().getId()))
						{
							if (item.getItemType() == ArmorType.HEAVY)
							{
								activeChar.sendMessage("You can use heavy only in Oly!");
								return;
							}
						}
					}
				...
				{
					IItemHandler handler = ItemHandler.getInstance().getHandler(item.getItem());
					if (handler == null)
					{
