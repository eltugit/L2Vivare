...
public class Attack extends L2GameServerPacket
{
    ...
    protected final void writeImpl()
    {
        ...
        boolean shouldSeeShots = (activeChar != null && !activeChar.getVarB("hideSSAnime"));
        Hit[] hits = _hits.toArray(new Hit[_hits.size()]);
...
//		writeLoc(_targetLoc);
        ...
        writeD(_attackerObjId);
        writeD(hits[0].getTargetId());
        writeD(hits[0].getDamage());
        writeC(shouldSeeShots ? hits[0].getFlags() : 0);
        writeLoc(_attackerLoc);
        writeH(hits.length - 1);
        for (int i = 1; i < hits.length; i++)
        {
            writeD(hits[i].getTargetId());
            writeD(hits[i].getDamage());
            writeC(shouldSeeShots ? hits[i].getFlags() : 0);
        }
        writeLoc(_targetLoc);
    }
}
