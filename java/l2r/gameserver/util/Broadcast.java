...
public final class Broadcast
{
	...
	public static void toKnownPlayersInRadius(L2Character character, L2GameServerPacket mov, int radius)
	{
		...
		Collection<L2PcInstance> plrs = character.getKnownList().getKnownPlayersInRadius(radius);
		for (L2PcInstance player : plrs)
		{
			player.sendPacket(mov);
		}
	...
	{
		toSelfAndKnownPlayersInRadius(character, mov, radius, false);
	}
	
	// To improve performance we are comparing values of radius^2 instead of calculating sqrt all the time
	public static void toSelfAndKnownPlayersInRadius(L2Character character, L2GameServerPacket mov, int radius, boolean ss)
	{
		if (radius < 0)
		{
			radius = 600;
		...
		for (L2PcInstance player : plrs)
		{
			if (player != null)
			{
				if (ss && player.getVarB("hideSSAnime") && ((mov.getClass() == MagicSkillUse.class) || (mov.getClass() == MagicSkillLaunched.class)))
				{
					continue;
				}
	...
	public static void sendMultisellPath(L2PcInstance activeChar, String message)
	{
		if (!Config.DEBUG_MULTISELL_DIR)
		{
			return;
		}
		
		sendFilePath(activeChar, message, "MULTISELL");
	}
	
	public static void sendHtmlPath(L2PcInstance activeChar, String message)
	{
		if (!Config.DEBUG_HTML_DIR)
		{
			return;
		}
		
		sendFilePath(activeChar, message, "HTML");
	}
	
	public static void sendUsedBypass(L2PcInstance activeChar, String message)
	{
		if (!Config.DEBUG_BYPASS_DIR)
		{
			return;
		}
		
		sendFilePath(activeChar, message, "BYPASS");
	}
	
	public static void sendFilePath(L2PcInstance activeChar, String message, String prefix)
	{
		if ((activeChar != null) && (activeChar.isGM() || activeChar.isDebug()))
		{
			activeChar.sendPacket(new CreatureSay(0, Say2.ALL, prefix, message));
		}
	}
}