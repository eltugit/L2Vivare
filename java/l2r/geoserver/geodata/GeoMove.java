...
public class GeoMove {
    ...
    private static List<Location> findPath(int x, int y, int z, Location target, L2Object obj, boolean showTrace, int geoIndex) {
        if (Math.abs(z - target.getZ()) > 256)
            return Collections.emptyList();

        z = GeoEngine.getHeight(x, y, z, geoIndex);
        target.setZ(GeoEngine.getHeight(target, geoIndex));
        final PathFind n = new PathFind(x, y, z, target.getX(), target.getY(), target.getZ(), obj, geoIndex);
        ...
        if (showTrace && obj.isPlayer() && ((L2PcInstance) obj).getVarB("get_trace", false)) {
    ...
    public static List<List<Location>> findMovePath(int x, int y, int z, Location target, L2Object obj, boolean showTrace, int geoIndex) {
        try {
            return getNodePath(findPath(x, y, z, target, obj, showTrace, geoIndex), geoIndex);
        } catch (Exception e) {
            Log.error("Error while finding move path from x: " + x + " y: " + y + " z: " + z + " to location: " + target + " at geoIndex: " + geoIndex);
        }
