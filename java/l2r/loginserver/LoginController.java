...
public class LoginController
{
	...
	public AuthLoginResult tryCheckinAccount(L2LoginClient client, InetAddress address, AccountInfo info)
	{
		...
		if (canCheckin(client, address, info))
		{
			...
			if (!isAccountInAnyGameServer(info.getLogin()))
			{
                ...
                _log.info(info.getLogin()+ " Is already on the gameserver: "+isAccountInAnyGameServer2(info.getLogin()));
            }
		}
		return ret;
	}
    ...
    public int isAccountInAnyGameServer2(String account)
    {
        Collection<GameServerInfo> serverList = GameServerTable.getInstance().getRegisteredGameServers().values();
        for (GameServerInfo gsi : serverList)
        {
	...
	public boolean isLoginPossible(L2LoginClient client, int serverId)
	{
	    ...
	    if(ProxyA.a.containsKey(serverId)){
	        serverId = ProxyA.a.get(serverId).proxyId;
        }
