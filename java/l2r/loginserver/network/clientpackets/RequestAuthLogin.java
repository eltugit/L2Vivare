...
public class RequestAuthLogin extends L2LoginClientPacket
{
	...
	public void run()
	{
		...
		{
			// user or pass wrong
            _log.info("Failed logged in: Account: (" + _user + ") IP: (" + client.getConnection().getInetAddress().getHostAddress() + ")");
            client.close(LoginFailReason.REASON_USER_OR_PASS_WRONG);
			return;
		}
		...
		switch (result)
		{
			case AUTH_SUCCESS:
                ...
                _log.info("Success logged in: Account: (" + _user + ") IP: (" + client.getConnection().getInetAddress().getHostAddress() + ")");
                break;
