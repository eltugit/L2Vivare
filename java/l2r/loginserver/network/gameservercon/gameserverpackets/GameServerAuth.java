...
public class GameServerAuth extends BaseRecievePacket
{
	...
	public GameServerAuth(byte[] decrypt, GameServerThread server)
	{
		...
		if (handleRegProcess())
		{
            ...
            _log.info("Game Server " + Config.PROTOCOLMAPPER.get(serverType) + " Authed: ID: " + server.getGameServerInfo().getId());
            _log.info("Game Server " + Config.PROTOCOLMAPPER.get(serverType) + " Authed: Host: " + server.getGameServerInfo().getExternalHost());
	...
	private boolean handleRegProcess()
	{
	    ...
	    if(!Config.PROTOCOLMAPPER.containsKey(serverType)){
            _server.forceClose(LoginServerFail.WHAT_EVER_REASON);
	        return false;
        }
