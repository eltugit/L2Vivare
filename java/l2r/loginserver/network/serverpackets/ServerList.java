...
import l2r.loginserver.AddressHolder;
...
public final class ServerList extends L2LoginServerPacket
{
    ...
    class ServerData
    {
        ...
        ServerData(L2LoginClient client, GameServerInfo gsi)
        {
            ...
            _serverId = gsi.getId();
            this._charsOnServer = ((client.getCharsOnServ() != null) ? client.getCharsOnServ().size() : 0);
        }
    }
...
//        System.out.println("IP is: " + client.getConnection().getInetAddress() + " / " + client.getAccount());
        AddressHolder.addHolder.put(client.getAccount(), client.getConnection().getInetAddress());
	}
	...
	{
		writeC(0x04);
		writeC(_servers.size() + ProxyA.size());
		writeC(_lastServer);
		for (ServerData server : _servers)
		{
            ...
            for (ProxyB proxyB : ProxyA.a(server._serverId)) {
                writeC(proxyB.serverId); // server id

                writeC(proxyB.ips[0] & 0xff);
                writeC(proxyB.ips[1] & 0xff);
                writeC(proxyB.ips[2] & 0xff);
                writeC(proxyB.ips[3] & 0xff);
                writeD(server._port);
                writeC(server._ageLimit); // Age Limit 0, 15, 18
                writeC(server._pvp ? 0x01 : 0x00);
                writeH(server._currentPlayers);
        ...
        writeH(164); // unknown
        writeC(_charsOnServers == null? 0 : _charsOnServers.size());
        for (ServerData server : _servers)
        {
            writeC(server._serverId);
            writeC(server._charsOnServer);
            writeC(0);
        }
	}
}
