 ...
 */
package scripts.ai.grandboss.Baium;
...
public final class Baium extends AbstractNpcAI {
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		switch (event) {
			...
			case "enter": {
				...
				} else if (getStatus() == IN_FIGHT && _is_locked) {
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon) {
		if (zone.isCharacterInZone(killer)) {
			...
			long respawnTime;
			if (GrandBossesConfig.BAIUM_CUSTOM_SPAWN_ENABLED && GrandBossesConfig.FindNext(GrandBossesConfig.BAIUM_CUSTOM_SPAWN_TIMES) != null)
				respawnTime = GrandBossesConfig.FindNext(GrandBossesConfig.BAIUM_CUSTOM_SPAWN_TIMES).getTimeInMillis() - System.currentTimeMillis();
			else {
				respawnTime = (Config.BAIUM_SPAWN_INTERVAL + getRandom(-Config.BAIUM_SPAWN_RANDOM, Config.BAIUM_SPAWN_RANDOM)) * 3600000;
			}
	...
	private static final String MSG = "{N} has {P}% HP. The entrance to the boss is available again! There are only {S} seconds to enter.";

	private synchronized void checkHp(L2Npc npc) {
		if (npc.getId() == BAIUM) {
			if (_is_locked) {
				double percent = 0;
				switch (_phase) {
				...
				if (percent > 0) {
					if (npc.getCurrentHp() < npc.getMaxHp() * percent) {
						_is_locked = false;
						_phase++;
						final int seconds = 360;
						final String msg = MSG.replace("{N}", npc.getName()).replace("{P}", "" + (int) (percent * 100)).replace("{S}", "" + seconds);
						GabUtils.yellowBroadcast(msg, "");
