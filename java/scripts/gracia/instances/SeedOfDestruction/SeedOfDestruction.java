 ...
 */
package scripts.gracia.instances.SeedOfDestruction;
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof SOD1World)
		{
			...
			else if (event.equalsIgnoreCase("BodyGuardThink"))
			{
				...
				if (mostHate != null)
				{
					double dist = Util.calculateDistance(mostHate.getDestination().getX(), mostHate.getDestination().getY(), 0, npc.getSpawn().getX(), npc.getSpawn().getY(), 0, false, false);
					if (dist > 900)
					{
