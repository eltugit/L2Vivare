 ...
 */
package scripts.handlers.chathandlers;
...
public class ChatShout implements IChatHandler
{
	...
	public void handleChat(int type, L2PcInstance activeChar, String target, String text)
	{
        ...
        if(GabUtils.blockAllChat(activeChar)) {
            return;
        }

        if(GabConfig.CHAT_SHOUT && activeChar.getPvpKills() < GabConfig.PVPS_TO_USE_CHAT_SHOUT){
            activeChar.sendMessage(String.format("You need at least %d pvp kills to use this chat", GabConfig.PVPS_TO_USE_CHAT_SHOUT));
            return;
        }
