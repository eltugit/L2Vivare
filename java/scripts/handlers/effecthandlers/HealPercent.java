	...
	public boolean onStart()
	{
		...
		try
		{
			if (getEffector().getObjectId() != target.getObjectId())
			{
                ...
                if(target instanceof L2PcInstance && getEffector() instanceof L2PcInstance){
                    getEffector().getActingPlayer().getMuseumPlayer().addData("healing_given", (long) amount);
                    target.getActingPlayer().getMuseumPlayer().addData("healing_received", (long) amount);
                }
			}
			else
			{
