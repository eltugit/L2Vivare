	...
	public boolean onActionTime()
	{
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(getSkill().getAffectRange()))
		{
			...
			if (cha.isAttackable() || cha.isPlayable())
			{
				...
				if(cha.isPlayer() && getEffector().isPlayer() && cha.isInParty() && getEffector().isInParty()){
					if(cha.getParty() == getEffector().getParty() || (cha.getParty().isInCommandChannel() && getEffector().getParty().isInCommandChannel() && cha.getParty().getCommandChannel() == getEffector().getParty().getCommandChannel()))
						continue;
				}

				if (getSkill().isOffensive() && !L2Skill.checkForAreaOffensiveSkills(getEffector(), cha, getSkill(), _srcInArena)) {
					if(Math.abs(getEffector().getZ() - ((L2PcInstance) getEffector()).getCurrentSkillWorldPosition().getZ()) > 350 || _actor.getDistance(_actor.getLocation()) > getSkill().getCastRange()) {
						continue;
					}
				}
		...
		if (!targets.isEmpty())
		{
			...
			for (L2Character target : targets)
			{
				...
				if (mdam > 0)
				{
					if (!target.isRaid() && Formulas.calcAtkBreak(target, mdam, getSkill()))
					{
						target.breakAttack();
						target.breakCast();
					}
