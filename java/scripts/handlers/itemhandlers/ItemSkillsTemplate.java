...
public class ItemSkillsTemplate implements IItemHandler
{
	...
	public boolean useItem(L2Playable playable, L2ItemInstance item, boolean forceUse)
	{
		...
		if (item.isPotion() && CSKOTHEvent.isPlayerParticipant(playable.getObjectId()) && !CSKOTHEvent.onPotionUse(playable.getObjectId())) {
			playable.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		if (item.isScroll() && CSKOTHEvent.isPlayerParticipant(playable.getObjectId()) && !CSKOTHEvent.onScrollUse(playable.getObjectId())) {
			playable.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
