 ...
 */
package scripts.handlers.skillhandlers;
			...
			switch (type)
			{
				case BETRAY:
				{
					...
					else
					{
                        ...
                        if((GabUtils.isInPvPInstance(activeChar) && GabUtils.isInPvPInstance(target)) && PvPZoneManager.isAnnonym()) {
                            sm.addCharName(ConfigPvPInstance.ANONYM_STRING);
                        }else if((GabConfig.OLY_VISUAL_RESTRICTION)&& (target instanceof L2PcInstance) &&((L2PcInstance) activeChar).isInOlympiadMode() && ((L2PcInstance) target).isInOlympiadMode()) {
