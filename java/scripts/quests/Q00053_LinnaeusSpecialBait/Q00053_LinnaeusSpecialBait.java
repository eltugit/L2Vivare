 ...
 */
package scripts.quests.Q00053_LinnaeusSpecialBait;
...
public class Q00053_LinnaeusSpecialBait extends Quest
{
	...
	// Items
	private static final int Nemeziz_DRAKE_HEART = 7624;
	...
	public Q00053_LinnaeusSpecialBait()
	{
		...
		addTalkId(LINNAEUS);
		addKillId(Nemeziz_DRAKE);
		registerQuestItems(Nemeziz_DRAKE_HEART);
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			case "31577-1.htm":
				...
				break;
			case "31577-3.htm":
				if (st.isCond(2) && (st.getQuestItemsCount(Nemeziz_DRAKE_HEART) >= 100))
				{
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (st.getQuestItemsCount(Nemeziz_DRAKE_HEART) < 100)
		{
			float chance = 33 * Config.RATE_QUEST_DROP;
			if (getRandom(100) < chance)
			{
				st.rewardItems(Nemeziz_DRAKE_HEART, 1);
