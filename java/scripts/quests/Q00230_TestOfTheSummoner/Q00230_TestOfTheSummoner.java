 ...
 */
package scripts.quests.Q00230_TestOfTheSummoner;
...
public final class Q00230_TestOfTheSummoner extends Quest
{
	...
	private static final int Nemeziz_BLOODSTONE = 3342;
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				...
				case FETTERED_SOUL:
				{
					if (!hasQuestItems(killer, GALATEAS_LETTER) && hasQuestItems(killer, LARAS_3RD_LIST))
					{
						giveItemRandomly(killer, npc, Nemeziz_BLOODSTONE, 6, 30, 1.0, true);
					}
					break;
				}
				case WINDSUS:
			...
			switch (npc.getId())
			{
				...
				case GROCER_LARA:
				{
					...
					else if (!hasQuestItems(player, GALATEAS_LETTER))
					{
						...
						{
							if ((getQuestItemsCount(player, BREKAORC_TOTEM) >= 30) && (getQuestItemsCount(player, Nemeziz_BLOODSTONE) >= 30))
							{
								takeItems(player, BREKAORC_TOTEM, -1);
								takeItems(player, Nemeziz_BLOODSTONE, -1);
