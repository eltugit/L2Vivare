 ...
 */
package scripts.quests.Q00386_StolenDignity;
...
public final class Q00386_StolenDignity extends Quest
{
	...
	// Monsters
	private static final int Nemeziz_DRAKE = 20670;
		...
		if ((qs != null) && (npc.getId() == WAREHOUSE_KEEPER_ROMP))
		{
			...
			switch (ask)
			{
				...
				case 18:
				{
					...
					return fillBoard(player, qs, getHtm(player, player.getHtmlPrefix(), "30843-13.html"));
				}
				case 19:
				case 20:
				case 21:
	...
	private String takeHtml(L2PcInstance player, QuestState qs, int num)
	{
		...
		if (!isSelectedBingoNumber(qs, num))
		{
			...
			if (i3 == 2)
			{
				html = getHtm(player, player.getHtmlPrefix(), "30843-14.html");
			}
			else if (i3 == 3)
			{
				html = getHtm(player, player.getHtmlPrefix(), "30843-16.html");
			}
			else if (i3 == 4)
			{
				html = getHtm(player, player.getHtmlPrefix(), "30843-18.html");
			}
			else if (i3 == 5)
			{
				html = getHtm(player, player.getHtmlPrefix(), "30843-20.html");
			}
		...
		if (i3 == 1)
		{
			html = getHtm(player, player.getHtmlPrefix(), "30843-15.html");
		}
		else if (i3 == 2)
		{
			html = getHtm(player, player.getHtmlPrefix(), "30843-17.html");
		}
		else if (i3 == 3)
		{
			html = getHtm(player, player.getHtmlPrefix(), "30843-19.html");
		}
		else if (i3 == 4)
		{
			html = getHtm(player, player.getHtmlPrefix(), "30843-21.html");
		}
		return fillBoard(player, qs, html);
	}
	...
	private String beforeReward(L2PcInstance player, QuestState qs, int num)
	{
		if (!isSelectedBingoNumber(qs, num))
		{
			...
			{
				reward(player, qs, 4);
				html = getHtm(player, player.getHtmlPrefix(), "30843-22.html");
			}
			else if ((i3 == 0) && (getBingoSelectCount(qs) == 6))
			{
				reward(player, qs, 10);
				html = getHtm(player, player.getHtmlPrefix(), "30843-24.html");
			}
			else
			{
				html = getHtm(player, player.getHtmlPrefix(), "30843-23.html");
			}
			return colorBoard(player, qs, html);
		}
		return fillBoard(player, qs, getHtm(player, player.getHtmlPrefix(), "30843-25.html"));
	}
