 ...
 */
package scripts.quests.Q00454_CompletelyLost;
				...
				if (qs.isCreated() && qs.isNowAvailable() && (player.getLevel() >= MIN_LEVEL))
				{
					...
					else
					{
						...
						if (leader.isInParty() && leader.getParty().containsPlayer(player))
						{
							qs.startQuest();
							qs.setMemoState(1);
							htmltext = getHtm(player, player.getHtmlPrefix(), "32738-04a.htm");
							htmltext = htmltext.replaceAll("leader", leader.getName());
						}
						else
						{
							htmltext = getHtm(player, player.getHtmlPrefix(), "32738-01b.htm");
							htmltext = htmltext.replaceAll("leader", leader.getName());
						}
					}
				}
			...
			case State.CREATED:
			{
				if (player.getLevel() >= MIN_LEVEL)
				{
					...
					else
					{
						...
						if (leader.isInParty() && leader.getParty().containsPlayer(player))
						{
							htmltext = getHtm(player, player.getHtmlPrefix(), "32738-01a.htm");
							htmltext = htmltext.replaceAll("leader", leader.getName());
